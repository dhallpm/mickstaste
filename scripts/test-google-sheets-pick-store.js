...
assert.equal(isGoogleSheetsSettledPickRow({ Status: 'Pending', 'Settlement Notes': 'The prop won.' }), true)
assert.equal(isGoogleSheetsSettledPickRow({ Status: 'Watchlist', 'Settlement Notes': 'The pick won.' }), true)
assert.equal(isGoogleSheetsSettledPickRow({ Status: 'Pass', 'Profit/Loss': '+1.00u' }), true)
...
assert.equal(inferGoogleSheetsSettlementResult({ 'Result Notes': 'The selection cashed.' }), 'Win')
