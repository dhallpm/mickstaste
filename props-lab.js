...
function propTodayKey(){ const d=new Date(); return `${d.getFullYear()}${String(d.getMonth()+1).padStart(2,'0')}${String(d.getDate()).padStart(2,'0')}`; }
...
function propParseCSV(text){ const rows=[]; let row=[],cur='',q=false; for(let i=0;i<text.length;i++){ const c=text[i],n=text[i+1]; if(c==='"'&&q&&n==='"'){cur+='"';i++;} else if(c==='"'){q=!q;} else if(c===','&&!q){row.push(cur);cur='';} else if((c==='\n'||c==='\r')&&!q){ if(cur!==''||row.length){row.push(cur);rows.push(row);row=[];cur='';} if(c==='\r'&&n==='\n')i++;} else cur+=c;} if(cur!==''||row.length){row.push(cur);rows.push(row);} return rows; }
...
async function propTodayRows(){ try{ const res=await fetch('/api/todays-picks',{cache:'no-store'}); if(!res.ok) throw new Error('Today picks feed '+res.status); const data=await res.json(); if(data.success===false) throw new Error(data.error||'Today picks feed unavailable'); return (Array.isArray(data.props)?data.props:[]).map(r=>({...r,_raw:r,_sourceTab:'Today Picks API',section:'props'})); }catch(err){ console.warn('Today picks feed failed closed:',err); return []; } }
...
function propIsClosed(row){ const status=propClean(`${row.status||''} ${row.result||''}`); return propIsWin(row)||propIsLoss(row)||propIsPush(row)||['graded','closed','settled','final','completed'].some(x=>status.includes(x)); }
function propIsActive(row){ const status=propClean(`${row.status||''} ${row.result||''}`); const day=propDateKey(row.date); if(!propHasText(row.pick)) return false; if(['void','cancelled','canceled','delete','removed'].some(x=>status.includes(x))) return false; if(propIsClosed(row)) return false; return day===propTodayKey() || ['active','posted','released','open'].some(x=>status.includes(x)); }
...
function propProfitLoss(row){ const result=propDisplayResult(row); const units=propNumber(row.units); const odds=propNumber(row.odds); if(result==='Push/Void') return '0.00u'; if(result==='Loss') return `-${units.toFixed(2)}u`; if(result!=='Win'||!units||!odds) return ''; const profit=odds>0?units*odds/100:units*100/Math.abs(odds); return `+${profit.toFixed(2)}u`; }
...
async function bootPropsLab(){
  ...
  try{
    ...
    const source='/api/todays-picks',totalCards=activeOpen.length; console.log('picksSource', source); console.log('pickCount', totalCards);
    ...
    renderPropGrid('propBetsGrid',propActive,16); renderPropResults('propResultsBody',propResults,100);
  ...
  }catch(e){ const grid=document.getElementById('propBetsGrid'); if(grid) grid.innerHTML='<div class="empty">No picks released yet.</div>'; console.error('Props Lab error:',e); }
}
bootPropsLab(); setInterval(bootPropsLab,30000);
