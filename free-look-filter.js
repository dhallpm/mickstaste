...
async function flAirtableRows(){const res=await fetch('/api/website-feed',{cache:'no-store'});if(!res.ok)throw new Error('Airtable website feed '+res.status);const data=await res.json();if(!data.success)throw new Error(data.error||'Airtable website feed unavailable');return(data.rows||[]).map(r=>({...r,_raw:r,_sourceTab:'Airtable Website Feed'}))}
async function flRows(gid,source){if(source==='Website Feed'){try{return await flAirtableRows()}catch(err){console.warn('Airtable free feed failed; using Google Sheets fallback:',err)}}const res=await fetch(flCsv(gid),{cache:'no-store'});const txt=await res.text();if(txt.toLowerCase().includes('<html'))throw new Error(source+' unavailable');return flObjects(flParseCSV(txt),source)}
