...
const FL_ALIASES={date:['Date'],sport:['Sport'],league:['League'],game:['Game','Matchup','Event'],pick:['Pick','Play','Selection'],betType:['Bet Type','Market','Type'],odds:['Odds','Price'],sportsbook:['Sportsbook','Book'],grade:['Grade'],units:['Units'],status:['Status'],result:['Result','Outcome'],profitLoss:['Profit/Loss','P/L','PL'],homePreview:['homePreview','Home Preview','Short Writeup','analysisPreviewShort'],writeup:['analysisPreview','Writeup','Write Up','Public Writeup'],access:['Access','Tier'],featured:['Featured','Featured?'],tag:['Tag','Tags','Category','Pick Tag','Card Tag'],releaseStatus:['Release Status','Release'],closingNumber:['Closing Number','Closing #'],bestNumber:['Best Number','Best #'],fullAnalysis:['Full Analysis'],marketNotes:['Market Notes'],injuryNotes:['Injury Notes'],confidence:['Confidence'],sourceVerification:['Source Verification']};
...
async function flTodayRows(){try{const res=await fetch('/api/todays-picks',{cache:'no-store'});if(!res.ok)throw new Error('Today picks feed '+res.status);const data=await res.json();if(data.success===false)throw new Error(data.error||'Today picks feed unavailable');return(Array.isArray(data.free)?data.free:[]).map(r=>({...r,_raw:r,_sourceTab:'Today Picks API'}))}catch(err){console.warn('Today picks feed failed closed:',err);return[]}}
async function flRows(gid,source){if(source==='Website Feed')return flTodayRows();const res=await fetch(flCsv(gid),{cache:'no-store'});const txt=await res.text();if(txt.toLowerCase().includes('<html'))throw new Error(source+' unavailable');return flObjects(flParseCSV(txt),source)}
...
function flTitle(v){return String(v||'').trim().replace(/^(?:(?:manual\s+review|watchlist|review|manual|hold|pending|test)\s*:\s*)+/i,'').replace(/\bmoney\s*line\b|\bmoneyline\b/gi,'ML').replace(/\s+/g,' ').trim()}
function flTeaser(r){const text=String(r.homePreview||r.writeup||'Public preview pending.').replace(/\s+/g,' ').trim();if(text.length<=280)return text;const clipped=text.slice(0,277),boundary=clipped.lastIndexOf(' ');return`${clipped.slice(0,boundary>=220?boundary:277).trim()}...`}
function flPickCard(r,teaserOnly=false){const tag=flTag(r),badge=flVipOverride(r)?'VIP A/A+ Prop Override':'FREE',grade=String(r.grade||'').trim(),gradeBadge=grade?`<div style="background:linear-gradient(135deg,#9b6d15,#ffe28a 54%,#b98821);color:#090909;padding:9px 11px;border-radius:10px;font-weight:1000">${flEsc(grade)}</div>`:'';return `<div class="pick-card"><div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start"><div><div style="color:var(--muted);font-size:11px;text-transform:uppercase;font-weight:1000;letter-spacing:.9px">${flEsc(r.league||r.sport||'Sports')} | ${flEsc(tag)}</div><div class="pick-title">${flEsc(flTitle(r.pick)||'Pick Pending')}</div><p style="color:var(--muted);line-height:1.5">${flEsc(r.game||'')}</p></div>${gradeBadge}</div><div class="metric-grid"><div class="metric"><strong>${flEsc(r.odds||'--')}</strong><span>Odds</span></div><div class="metric"><strong>${flEsc(r.sportsbook||'--')}</strong><span>Book</span></div><div class="metric"><strong>${flEsc(r.bestNumber||'--')}</strong><span>Best #</span></div><div class="metric"><strong>${flEsc(badge)}</strong><span>Card Type</span></div></div><p style="color:#e7dcc4;line-height:1.55;margin-top:14px">${flEsc(teaserOnly?flTeaser(r):(r.writeup||'Public writeup loading from Micks Picks.'))}</p></div>`}
function flRenderGrid(id,rows,limit=12){const el=document.getElementById(id);if(!el)return;const active=flDedupe(rows).filter(flActiveEligible).sort((a,b)=>flDateVal(b.date)-flDateVal(a.date)).slice(0,limit);el.innerHTML=active.length?active.map(flPickCard).join(''):'<div class="empty">No picks released yet.</div>'}
function flRenderPickOfDay(rows){const el=document.getElementById('freePickOfDayGrid');if(!el)return;const active=flDedupe(rows).filter(flActiveEligible);if(!active.length){el.innerHTML='<div class="empty">No picks released yet.</div>';return}const featured=active.filter(r=>flClean(r.featured)==='yes');const pool=(featured.length?featured:active).sort((a,b)=>flGradeRank(b.grade)-flGradeRank(a.grade)||flDateVal(b.date)-flDateVal(a.date));el.innerHTML=flPickCard(pool[0],true)}
