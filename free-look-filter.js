...
async function flTodayRows(){try{const res=await fetch('/data/todays-picks.json',{cache:'no-store'});if(!res.ok)throw new Error('Today picks file '+res.status);const data=await res.json();if(data.success===false)throw new Error(data.error||'Today picks file unavailable');return(Array.isArray(data.free)?data.free:[]).map(r=>({...r,_raw:r,_sourceTab:'Today Picks Static JSON'}))}catch(err){console.warn('Today picks static file unavailable:',err);return[]}}
