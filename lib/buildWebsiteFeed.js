...
export function cleanWebsiteRow(row = {}) {
  ...
  return {
    ...
    game: keyed.Game || keyed.Matchup || keyed.game || (isLotto ? keyed['Parlay Type'] || pick : ''),
    pick,
...
function lottoExclusionWarning(sourceRows = [], websiteRows = [], targetDate = new Date()) {
  ...
  const diagnostics = lottoRows.slice(0, 4).map(row => ({
    ...
    parlayType: firstText(row['Parlay Type']),
    status: firstText(row.Status, row['Display Status']),
    releaseStatus: firstText(row['Release Status'], row['Display Release Status']),
    result: firstText(row.Result, row.Outcome),
    archiveStatus: firstText(row['Archive Status']),
