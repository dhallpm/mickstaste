...
function newestFirst(a, b) {
  return newestTimestamp(b) - newestTimestamp(a) ||
    fullAnalysisLength(b) - fullAnalysisLength(a) ||
...
function newestTimestamp(row = {}) {
  const raw = firstText(row.updatedAt, row['Updated At'], row['Last Modified'], row.Timestamp, row.date, row.Date)
  const parsed = Date.parse(raw)
  return Number.isNaN(parsed) ? 0 : parsed
}

function fullAnalysisLength(row = {}) {
  return firstText(row.fullAnalysis, row['Full Analysis'], row.Analysis, row.analysis).length
}

function isExplicitNo(value = '') {
  return /^(?:no|false|0)$/i.test(compact(value))
}

function officialBetValue(row = {}) {
  return firstText(row['Official Bet'], row.officialBet, row.Official, row.official)
}

function releaseStatusValue(row = {}) {
  return firstText(row['Release Status'], row['Display Release Status'], row.releaseStatus, row.release)
}

function statusValue(row = {}) {
  return firstText(row.Status, row['Display Status'], row.status)
}

function isNoReleaseRow(row = {}) {
  const release = normalized(releaseStatusValue(row))
  return /\b(no release|not released|do not release|draft|held)\b/.test(release)
}

function isNoBetRow(row = {}) {
  const status = normalized(textFrom(row, ['Status', 'Display Status', 'Release Status', 'Category', 'Action']))
  return isExplicitNo(officialBetValue(row)) || /\b(no bet|do not bet)\b/.test(status)
}

function isDuplicateOrArchived(row = {}) {
  const marker = normalized(textFrom(row, ['Status', 'Display Status', 'Release Status', 'Archive Status', 'Action', 'Tags']))
  const statusMarker = normalized(textFrom(row, ['Status', 'Display Status', 'Release Status', 'Action', 'Tags']))
  const archive = normalized(row['Archive Status'])
  const archived = archive &&
    !/\b(not archived|not archive|unarchived|active|open|pending|no)\b/.test(archive) &&
    /\b(archived|archive complete|moved to archive)\b/.test(archive)
  return archived || /\bduplicate\b/.test(marker) || /\b(archive complete|moved to archive|archived)\b/.test(statusMarker)
}

function isIgnoredActiveRow(row = {}) {
  return isNoBetRow(row) || isNoReleaseRow(row) || isDuplicateOrArchived(row) || isFinalOrPass(row)
}

function sourceVipAccess(row = {}) {
  return /\b(vip|premium)\b/i.test(firstText(row.Access, row.access, row.Tier, row['Access Tier']))
}

function sourceVipReleased(row = {}) {
  return /^vip released$/i.test(compact(releaseStatusValue(row)))
}

function sourceVipStatusAllowed(row = {}) {
  return /^(?:pending|released|active)$/i.test(compact(statusValue(row)))
}
...
async function getSourceRows() {
  ...
  try {
    ...
    return {
      ...
      spreadsheetId: activeResult.spreadsheetId,
      warnings,
...
export function sourceRowVisible(row = {}, targetDate = new Date()) {
  ...
  if (isIgnoredActiveRow(row)) return false
  if (sourceVipAccess(row) && (!sourceVipStatusAllowed(row) || !sourceVipReleased(row))) return false
...
function fallbackPickKey(row = {}) {
  return [
    ...
    row.game,
    row.pick
  ].map(identityKey).filter(Boolean).join('|')
}
