...
export function cleanWebsiteRow(row = {}) {
  ...
  const pick = sanitizeCustomerFacingTitle(firstText(
    keyed.Pick,
    keyed.Selection,
    keyed.pick,
    keyed['Card Title'],
    keyed.Name,
    keyed.Title,
    isLotto ? keyed['Parlay Type'] : '',
    isLotto ? keyed['Parlay Name'] : ''
  ))
  const cardTitle = sanitizeCustomerFacingTitle(keyed['Card Title'] || pick || keyed.Selection || keyed.Name || keyed.Title || '')
  ...
  return {
    ...
    game: keyed.Game || keyed.Matchup || keyed.game || (isLotto ? firstText(keyed['Parlay Type'], cardTitle, pick, keyed.Legs) : ''),
    pick,
    betType: keyed['Bet Type'] || keyed.Type || keyed.Market || (isLotto ? 'Parlay' : ''),
    ...
    releaseStatus: keyed['Release Status'] ||
      ...
      (isLotto && /^active$/i.test(String(keyed.Status || '').trim()) && keyed.Odds !== '' && keyed.Odds !== undefined && keyed.Odds !== null && firstText(keyed.Sportsbook, keyed.Book)
        ? 'Released'
        : ''),
