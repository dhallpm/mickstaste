...
function manualTrigger(row = {}) {
  return truthy(row['Manual Submitted']) || truthy(row['Release Approved']) || /released/i.test(String(row['Release Status'] || ''))
}

function manualCompletion(row = {}) {
  const odds = text(row, ['Manual Odds', 'Odds'])
  const sportsbook = text(row, ['Manual Sportsbook', 'Sportsbook'])
  const line = text(row, ['Manual Line', 'Line', 'Best Number', 'Suggested Line'])
  const source = text(row, ['Manual Confirmed', 'Source Verification'])
  const errors = []
  if (!isAmericanOdds(odds)) errors.push('Manual odds/Odds must be numeric American odds.')
  if (!sportsbook) errors.push('Manual sportsbook/Sportsbook is required.')
  ...
  if (!source) errors.push('Source Verification or Manual Confirmed is required.')
...
export async function submitManualPicks(options = {}) {
  ...
  const candidates = resolved.records.filter(record => manualTrigger(record.fields || {}))
  ...
  for (const record of candidates) {
    ...
    const approved = truthy(fields['Release Approved']) || /released/i.test(String(fields['Release Status'] || ''))
    const nextFields = {
      Odds: completion.odds,
      Sportsbook: completion.sportsbook,
      'Best Number': completion.line || fields['Best Number'] || fields['Suggested Line'] || '',
      'Source Verification': completion.source,
      'Manual Odds Needed': false,
      'Sportsbook Needed': false,
      'Manual Submitted': truthy(fields['Manual Submitted']),
      'Manual Submit Time': fields['Manual Submit Time'] || new Date().toISOString(),
      ...
      ...(approved
        ...
        : {
          Status: fields.Status || 'Pregame',
          'Release Status': fields['Release Status'] || 'Held'
        })
    }
    updates.push({ id: record.id, fields: nextFields })
    normalized.push({ id: record.id, recordKey: fields['Record Key'] || '', approved })
  }
