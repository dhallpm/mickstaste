...
function manualReviewCard(candidate = {}, options = {}) {
  ...
  return normalizeAiCard({
    ...
    'Full Analysis': [
      'Held manual review card from Run Micks Picks.',
      summary,
      missingText ? `Missing fields: ${missingText}.` : '',
      'Do not release until manual odds, sportsbook, line, source verification, and approval are completed.'
    ].filter(Boolean).join('\n'),
...
export async function generateMicksPicks(options = {}) {
  ...
  routed.routeOutputs.manualReview = manualReviewPool
