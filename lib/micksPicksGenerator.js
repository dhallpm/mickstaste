...
async function callOpenAiEngine({ date, sport, league, mode, candidatePoolSize, access, framework, routing, propsSearch = false, allowRetry = false }) {
  ...
  error.retryUsed = attempts > 1
  error.parseFailures = parseFailures
  error.rawOpenAiPreview = previewText(responseText(lastPayload))
...
export async function generateMicksPicks(options = {}) {
  ...
  try {
    ...
    return finalize({
      ...
      ...(dryRun && error.rawOpenAiPreview ? { rawOpenAiPreview: error.rawOpenAiPreview } : {})
    })
  }
