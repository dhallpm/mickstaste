...
function frameworkPrompt(framework = [], context = {}) {
  ...
  const relevant = new Set(['README.md', 'a-grade-hunt.md', A_GRADE_HUNT_RULES_FILE, POST_JUNE8_ADJUSTMENTS_MD_FILE, POST_JUNE8_ADJUSTMENTS_FILE, 'bankroll.md', 'data-integrity.md', 'closing-odds.md'])
...
function decorateCandidate(candidate = {}) {
  const preGatedCandidate = applyPostJune8Adjustments(candidate)
  const aGatedCandidate = applyAGradeGate(preGatedCandidate)
  const postJune8Candidate = applyPostJune8Adjustments(aGatedCandidate)
