...
function frameworkPrompt(framework = [], context = {}) {
  ...
  if (ultraFastMode) {
    return [
      ...
      '- Use A/B/C/Pass grading, implied probability, estimated true probability, EV edge, best number, no-bet cutoff, confidence, and units.',
  ...
  if (fastMode) {
    ...
    if (league.includes('wnba')) addSnippet('wnba.md', 1200)
...
function openAiRequestBody({ model, date, sport, league, mode, candidatePoolSize, access, framework, routing, propsSearch = false, retry = false, fastMode = true, ultraFastMode = false }) {
  const target = propsSearch ? Math.min(Number(candidatePoolSize) || 4, ultraFastMode ? 4 : 6) : 2
  ...
  return {
    ...
    input: [{
      ...
      content: [{
        ...
        text: [
          ...
          scope,
          '',
          propsSearch ? (fastMode ? 'Compact Micks Picks framework:' : 'Full Micks Picks framework:') : 'Tiny Micks Picks WNBA rules:',
          propsSearch ? frameworkPrompt(framework, {
            ...
            '- WNBA board only: sides, totals, moneylines.',
            '- Return at most 2 candidates with verified current American odds like -110 or +125, a real sportsbook/book label, line, and source URL/text.',
            ...
            '- Do not use prediction articles as candidate sources unless they include a current book-labelled line and price.',
            '- Put rawCandidatePool first. Put sourcesUsed after candidates. Use short source titles.',
            '- Use matchup/market/injury context only as a short summary.',
            '- If odds, sportsbook, line, game, pick, or source verification are missing, put the idea in passes.',
          ...
          `- rawCandidatePool must contain at most ${propsSearch ? (ultraFastMode ? 3 : 4) : 2} real candidates. Backend code will normalize, score, and route after JSON parses.`,
          '- Each candidate must include only: Game, Pick, Bet Type, Odds, Sportsbook, Line, Source Verification, sourceConfidence, manualConfirmationRequired, summary.',
          ...
          '- If odds are not an exact American price or sportsbook is only aggregate/consensus, put the idea in passes.',
    ...
    tool_choice: 'required',
    reasoning: { effort: 'low' },
    max_output_tokens: Math.min(maxOutputTokens(), propsSearch ? 700 : (ultraFastMode ? 450 : 600)),
    text: {
      format: {
        type: 'json_schema',
...
async function callOpenAiEngine({ date, sport, league, mode, candidatePoolSize, access, framework, routing, propsSearch = false, allowRetry = false, fastMode = true }) {
  ...
  try {
    ...
    const safeError = buildJsonParseError({
      ...
      retryUsed,
      parseFailures,
      parseErrorType: parseErrorType || (retryUsed ? 'repair_exception' : 'invalid_json'),
      rawOpenAiPreview: previewText(lastRawText || firstRawText),
...
function coreSourcePassReason(candidate = {}) {
  ...
  if (!/^[+-]?\d{2,5}$/.test(String(odds || '').trim())) return 'Odds were not verified as an exact American price.'
  if (!sportsbook || hasWord(/\b(aggregate|consensus|not verified|unknown|pending|multiple)\b/i, sportsbook)) return 'Sportsbook was not verified as a real book label.'
  if (!textFrom(candidate, ['Game', 'game'])) return 'Game was missing.'
  if (!textFrom(candidate, ['Pick', 'pick'])) return 'Pick was missing.'
  if (!textFrom(candidate, ['Line', 'line'])) return 'Line was missing.'
  if (!textFrom(candidate, ['Source Verification', 'sourceVerification'])) return 'Source verification was missing.'
  return ''
}
...
function routeCandidatePool(rawCandidates = [], options = {}) {
  ...
  const releaseEligible = candidatePool.filter(candidate => !hasWord(/\bpass\b/i, candidate['Release Gate']) && routeGateOk(candidate))
