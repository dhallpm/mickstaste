...
function rateBudgetFromCache(cache, config, nowMs, cachedResultUsed = false) {
...
async function rateGuard(options = {}, mode = '') {
  ...
  if (!forceRefresh && cached?.result && Number.isFinite(cachedAtMs) && nowMs - cachedAtMs < windowMs) {
    return {
      allowed: false,
      cached: true,
      cache,
      ...
      result: {
        ...cached.result,
        fromCache: true,
        rateBudget: rateBudgetFromCache(cache, config, nowMs, true)
      }
    }
  }

  const budget = rateBudgetFromCache(cache, config, nowMs, false)
  const nextAllowedMs = Date.parse(budget.nextAllowedRunAt)
  if (budget.runsRemaining <= 0 || (Number.isFinite(nextAllowedMs) && nextAllowedMs > nowMs)) {
    return {
      allowed: false,
      cached: false,
      cache,
  ...
  return { allowed: true, cached: false, cache, key, rateBudget: budget, config, nowMs }
}
...
async function saveRateGuardResult(guard, result = {}) {
  ...
  return {
    ...result,
    rateBudget: rateBudgetFromCache(cache, guard.config, Date.now(), false)
  }
}
...
async function callOpenAiEngine({ date, sport, league, mode, candidatePoolSize, access, framework, routing, propsSearch = false, allowRetry = false }) {
  ...
  const attempts = propsSearch && allowRetry ? 2 : 1
  let lastPayload = {}
...
export async function generateMicksPicks(options = {}) {
  ...
  try {
    ...
    return finalize({
      ...
      propsStatus: { requested: routing.includeProps, status: routing.includeProps ? 'incomplete' : 'not_requested', rawFound: 0, verified: 0, missing: ['OpenAI engine failed'], reason: 'OpenAI engine failed before routing.' },
      ...
      watchlistCount: 0,
      passCount: 0,
      tokenBudget: { openAiRuns: 1, propsSecondPass: false, candidatePoolTarget: routing.candidatePoolSize, maxRuns: maxOpenAiCalls, deepMode, perRunBudget: tokenBudgetLimit(), maxOutputTokens: maxOutputTokens() },
      sourceAcquisition: emptySourceAcquisition(routing, 'OpenAI engine failed before source acquisition.'),
      passes: [],
