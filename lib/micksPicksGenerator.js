...
function openAiRequestBody({ model, date, sport, league, mode, candidatePoolSize, access, framework, routing, propsSearch = false, retry = false }) {
  ...
  return {
    ...
    input: [{
      ...
      content: [{
        ...
        text: [
          ...
          `- Build one shared ranked candidate pool of ${retry ? Math.min(12, candidatePoolSize) : candidatePoolSize} legs if enough verified candidates exist.`,
          ...
          '- Each candidate must include Category, game, pick, odds, sportsbook or clear source status, grade, confidence, EV edge when available, best number, no-bet cutoff, risk/variance note, source verification, conflict/correlation tags, Eligible Routes, Release Gate, sourceConfidence, missingFields, and manualConfirmationRequired.',
          ...
          retry ? '- Output compact JSON only using the smaller retry schema.' : '- Output JSON only using the requested schema.'
        ].join('\n')
      }]
    }],
    ...
    text: {
      format: {
        type: 'json_schema',
        name: retry ? 'micks_picks_cards_compact' : 'micks_picks_cards',
        strict: true,
...
async function callOpenAiEngine({ date, sport, league, mode, candidatePoolSize, access, framework, routing, propsSearch = false, allowRetry = false }) {
  ...
  error.openAiRuns = attempts
  error.parseFailures = parseFailures
  error.details = lastPayload?.id ? { responseId: lastPayload.id } : undefined
  throw error
}
...
export async function generateMicksPicks(options = {}) {
  ...
  try {
    ...
    return finalize({
      ...
      watchlistCount: 0,
      passCount: 0,
      tokenBudget: { openAiRuns, propsSecondPass: false, propsSecondPassSkippedReason: failureStage === 'jsonParse' ? 'Core JSON parse failed; props/lotto refinement skipped.' : '', candidatePoolTarget: routing.candidatePoolSize, maxRuns: maxOpenAiCalls, deepMode, perRunBudget: tokenBudgetLimit(), maxOutputTokens: maxOutputTokens() },
