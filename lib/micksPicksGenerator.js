...
function configuredProvider() {
  if (process.env.MICKS_PICKS_PROVIDER_JSON) return { type: 'json', label: 'MICKS_PICKS_PROVIDER_JSON' }
  if (process.env.MICKS_PICKS_PROVIDER_URL) return { type: 'url', url: process.env.MICKS_PICKS_PROVIDER_URL, label: 'MICKS_PICKS_PROVIDER_URL' }
  if (process.env.MICKS_PICKS_DATA_URL) return { type: 'url', url: process.env.MICKS_PICKS_DATA_URL, label: 'MICKS_PICKS_DATA_URL' }
  return null
}
...
function appearsSampleCandidate(row = {}) {
  const values = [
    get(row, ['game', 'Game', 'matchup', 'Matchup', 'event', 'Event']),
    get(row, ['pick', 'Pick', 'selection', 'Selection']),
    get(row, ['sourceVerification', 'Source Verification', 'source', 'Source']),
    get(row, ['writeup', 'Writeup']),
    get(row, ['fullAnalysis', 'Full Analysis', 'analysis', 'Analysis'])
  ]
  return values.some(hasSampleText)
}

function appearsSamplePick(card = {}) {
  return [card.game, card.pick, card.sourceVerification, card.writeup, card.fullAnalysis]
    .some(hasSampleText)
}
...
export async function generateMicksPicks(options = {}) {
  ...
  const providerStatus = provider
    ? { configured: true, type: provider.type, source: provider.label }
    : { configured: false }
  ...
  if (!provider) {
    return {
      ...
      providerStatus,
      airtableButtonUrl: '/api/micks-admin?action=run-micks-picks&mode=publish&maxPicks=3&access=auto',
  ...
  const warnings = []
  const sampleDataDetected = scopedRows.some(appearsSampleCandidate) || generated.some(appearsSamplePick)
  if (mode === 'publish' && sampleDataDetected && !allowSample) {
    return {
      success: false,
      error: 'Provider data appears to be sample/test data. Replace provider feed before publishing real picks.',
      providerStatus,
