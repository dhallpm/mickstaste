...
function openAiRequestBody({ model, date, sport, league, mode, candidatePoolSize, access, framework, routing, propsSearch = false, retry = false, fastMode = true, ultraFastMode = false }) {
  const target = Math.min(Number(candidatePoolSize) || 4, ultraFastMode ? 4 : 6)
  const scope = [
    `Date: ${date}`,
    sport ? `Sport filter: ${sport}` : '',
    league ? `League filter: ${league}` : '',
    ...
    `Routing quotas: ${JSON.stringify({ ...routing, candidatePoolSize: target })}`,
  ...
  return {
    ...
    input: [{
      ...
      content: [{
        ...
        text: [
          ...
          scope,
          '',
          fastMode ? 'Compact Micks Picks framework:' : 'Full Micks Picks framework:',
          ...
          `- rawCandidatePool must contain at most ${ultraFastMode ? 3 : 4} real candidates. Backend code will normalize, score, and route after JSON parses.`,
    ...
    tool_choice: 'required',
    reasoning: { effort: 'low' },
    max_output_tokens: Math.min(maxOutputTokens(), ultraFastMode ? 500 : 700),
    text: {
      format: {
        type: 'json_schema',
