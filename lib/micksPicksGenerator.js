...
function routeCandidatePool(rawCandidates = [], options = {}) {
  ...
  const releaseEligible = decorated.filter(candidate => !hasWord(/\bpass\b/i, candidate['Release Gate']))
  const invalidOrPass = decorated.filter(candidate => hasWord(/\bpass\b/i, candidate['Release Gate']))
  ...
  const parlayLegs = normalized.filter(candidate => !['longshot', 'prop'].includes(candidate._type) && positiveEdge(candidate))
  const cards = []
  const warnings = []
  const passes = [
    ...
    ...invalidOrPass.map(candidate => ({
      ...
      Reason: candidate['Release Gate'] || 'Pass',
      'Source Verification': textFrom(candidate, ['Source Verification', 'sourceVerification']) || 'Source not verified.'
    }))
  ]
  const routeOutputs = {
