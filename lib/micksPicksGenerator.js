...
const PROPS_PASS_RETRY_OUTPUT_TOKEN_LIMIT = 500
...
function maxOutputTokens() {
  return intValue(process.env.MICKS_PICKS_MAX_OUTPUT_TOKENS, DEFAULT_MAX_OUTPUT_TOKENS, 200, 4000)
}

function openAiTimeoutMs() {
  return intValue(process.env.MICKS_PICKS_OPENAI_TIMEOUT_MS, DEFAULT_OPENAI_TIMEOUT_MS, 1000, 55000)
}
...
function parseOpenAiJsonText(text = '') {
  ...
  try {
    return { parsed: JSON.parse(cleanJsonText(text)), parseErrorType: '' }
  } catch {
    const extracted = extractFirstJsonObject(text)
    if (extracted) return { parsed: extracted, parseErrorType: 'extracted_balanced_json' }
...
function responseSources(payload = {}, aiJson = {}) {
  const urls = new Map()
  for (const source of aiJson.sourcesUsed || aiJson.sources || []) {
...
function strongGrade(candidate = {}) {
  return hasWord(/\b(A|A-|B\+|B)\b/i, String(textFrom(candidate, ['Grade', 'grade']) || ''))
}
...
function decorateCandidate(candidate = {}) {
  const normalizedGate = normalizeReleaseGate(candidate)
  const confidence = sourceConfidence(candidate)
  const manualRequired = manualConfirmationRequired(candidate)
  return {
    ...candidate,
    'Eligible Routes': eligibleRoutes(candidate),
    'Original Release Gate': textFrom(candidate, ['Release Gate', 'releaseGate']) || '',
    'Release Gate': normalizedGate,
    sourceConfidence: confidence,
    missingFields: missingCandidateFields(candidate),
...
function vipEligible(candidate = {}) {
  ...
  if (!strongGrade(candidate)) return false
...
function aiJsonSchema() {
  ...
  return {
    ...
    required: ['sourcesUsed', 'rawCandidatePool', 'passes', 'warnings']
  }
}

function compactAiJsonSchema() {
  return aiJsonSchema()
}
...
function openAiRequestBody({ model, date, sport, league, mode, candidatePoolSize, access, framework, routing, propsSearch = false, retry = false }) {
  const scope = [
    `Date: ${date}`,
    sport ? `Sport filter: ${sport}` : '',
    league ? `League filter: ${league}` : '',
  ...
  return {
    ...
    input: [{
      ...
      content: [{
        ...
        text: [
          ...
          '- Treat source acquisition as targeted compact passes inside this run: board odds pass for book/aggregate odds, injury pass for official/reliable injury context, and props pass only when props are requested.',
          '- Return only sourcesUsed, rawCandidatePool, passes, and warnings. Do not return routed cards.',
          '- rawCandidatePool must contain at most 4 real candidates. Backend code will normalize, score, and route after JSON parses.',
          '- Each candidate must include game, pick, bet type, category, odds, sportsbook or source status, line, source verification, sourceConfidence, missingFields, manualConfirmationRequired, and a short summary.',
          ...
          '- Medium-confidence candidates may only be Review Eligible when manualConfirmationRequired is true. Low-confidence candidates must be Watchlist or Pass.',
          '- Use real current data only. Do not invent games, odds, injuries, sportsbooks, or fake teams.',
          '- If a candidate lacks verified odds, game, pick, or source verification, put it in passes instead of rawCandidatePool.',
    ...
    max_output_tokens: retry ? Math.min(maxOutputTokens(), 800) : maxOutputTokens(),
    text: {
      format: {
        type: 'json_schema',
        name: retry ? 'micks_picks_source_pool_retry' : 'micks_picks_source_pool',
        strict: true,
        schema: retry ? compactAiJsonSchema() : aiJsonSchema()
      }
    }
  }
}
  ...
  return {
    ...
    text: {
      format: {
        ...
        strict: true,
        schema: propsSearch ? propsJsonSchema() : aiJsonSchema()
      }
    }
  }
}

async function callOpenAiEngine({ date, sport, league, mode, candidatePoolSize, access, framework, routing, propsSearch = false, allowRetry = false }) {
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    ...
    try {
      response = await fetch(OPENAI_RESPONSES_URL, {
        ...
        body: JSON.stringify(attempt > 0
          ? openAiRepairRequestBody({ model, rawText: firstRawText, propsSearch })
          : propsSearch
          ? openAiPropsRequestBody({ model, date, sport, league, routing, retry: attempt > 0 })
          : openAiRequestBody({ model, date, sport, league, mode, candidatePoolSize, access, framework, routing, propsSearch, retry: attempt > 0 }))
      })
    } catch (error) {
      const timedOut = error?.name === 'AbortError'
      const wrapped = new Error(timedOut
        ? `OpenAI Micks Picks engine timed out after ${openAiTimeoutMs()}ms.`
    ...
    lastRawText = text
    if (attempt === 0) firstRawText = text
    const parsedJson = parseOpenAiJsonText(text)
    ...
    if (parsedJson.parsed) {
      return {
        ...
        searchesRun: Number(parsedJson.parsed.searchesRun || searchesRun(payload) || 0),
        sourcesUsed: responseSources(payload, parsedJson.parsed),
  ...
  const error = new Error('OpenAI Micks Picks engine returned invalid JSON after strict JSON retry.')
  error.statusCode = 200
  error.stage = 'jsonParse'
  error.openAiRuns = parseFailures > 1 ? 2 : 1
  error.retryUsed = parseFailures > 1
  error.parseFailures = parseFailures
  error.parseErrorType = parseErrorType || 'invalid_json'
  error.rawOpenAiPreview = previewText(lastRawText)
  error.responseId = responseId || lastPayload?.id || ''
...
function routeCandidatePool(rawCandidates = [], options = {}) {
  ...
  const candidatePool = decorated.filter(candidate => !hasWord(/\bpass\b/i, candidate['Release Gate']) && !missingCandidateFields(candidate).length)
  const releaseEligible = candidatePool.filter(routeGateOk)
  ...
  if (routing.maxVipPicks && !vip.length) {
    vipStatus.reason = 'No positive-edge VIP candidates passed release/review gate.'
...
export async function generateMicksPicks(options = {}) {
  ...
  const coreOpenAiRuns = 1 + (engine.retries || 0)
  const tokenBudget = {
  ...
  if (routing.includeProps && routed.propsStatus.verified === 0) {
    ...
    if (!deepMode || maxOpenAiCalls <= tokenBudget.openAiRuns) {
      tokenBudget.propsSecondPassSkippedReason = 'Props refinement skipped because deepMode=1 is required for a second OpenAI/web-search call.'
  ...
  let ingest
  try {
    ingest = await ingestPicksToAirtable({ date, picks: validated }, { dryRun: false })
