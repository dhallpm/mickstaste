...
async function callOpenAiEngine({ date, sport, league, mode, candidatePoolSize, access, framework, routing, propsSearch = false }) {
  ...
  const response = await fetch(OPENAI_RESPONSES_URL, {
    ...
    body: JSON.stringify(openAiRequestBody({ model, date, sport, league, mode, candidatePoolSize, access, framework, routing, propsSearch }))
  })
...
function routeCandidatePool(rawCandidates = [], options = {}) {
  ...
  const straight = routeBase.filter(candidate => candidate._type === 'straight' && positiveEdge(candidate))
  const props = routeBase.filter(candidate => candidate._type === 'prop' && positiveEdge(candidate) && verifiedProp(candidate))
  const longshots = routeBase.filter(candidate => candidate._type === 'longshot' && positiveEdge(candidate))
  const parlayLegs = routeBase.filter(candidate => !['longshot', 'prop'].includes(candidate._type) && positiveEdge(candidate))
  const reviewLottoLegs = normalized.filter(candidate => !['longshot', 'prop'].includes(candidate._type) && positiveEdge(candidate))
  const reviewPropLegs = normalized.filter(candidate => candidate._type === 'prop' && positiveEdge(candidate) && verifiedProp(candidate))
  const cards = []
  const warnings = []
  const passes = [
  ...
  if (routing.includeProps) {
    ...
    if (routing.maxProps && !topProps.length) warnings.push(`No verified prop candidates were available for Props Lab: ${propsStatus.missing.join(', ')}.`)
  }
