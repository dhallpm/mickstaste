...
const POST_JUNE8_ADJUSTMENTS_FILE = 'current/post-june8-adjustments.json'
const POST_JUNE8_ADJUSTMENTS_MD_FILE = 'current/post-june8-adjustments.md'
const WORLD_CUP_PATH_LEVERAGE_FILE = 'current/world-cup-path-leverage.json'
const ALLOWED_MODES = new Set(['draft', 'review', 'publish', 'props', 'longshots'])
const ALLOWED_ACCESS = new Set(['free', 'vip', 'premium', 'auto'])
const SUPPORTED_RUN_LEAGUES = ['MLB', 'NHL', 'NBA', 'WNBA', 'UFC', 'SOCCER', 'WORLD_CUP']
const FRAMEWORK_FILE_ORDER = [
  'README.md',
  'a-grade-hunt.md',
  A_GRADE_HUNT_RULES_FILE,
  WORLD_CUP_PATH_LEVERAGE_FILE,
  POST_JUNE8_ADJUSTMENTS_MD_FILE,
  POST_JUNE8_ADJUSTMENTS_FILE,
  'mlb.md',
  'nba.md',
  'wnba.md',
  'ufc.md',
...
const POST_JUNE8_FIELDS = [
  'Park/Weather Risk',
  'Blow-Up Risk',
  'Volatility Capped',
  'Tags'
]
const POST_JUNE8_TAGS = [
  'Extreme Run Environment',
  'Volatile Total Cap',
  'Park/Weather Conflict',
  'Role-Stable Prop',
  'A-Hunt Candidate',
  'B+ Near-A'
]
...
const WORLD_CUP_OUTPUT_SECTION_NAMES = [
  'World Cup Path Watchlist',
  'Group Winner Edge Board',
  'Live Group-Position Triggers',
  'Host-Field Adjustment Notes',
  'Bracket Collision Alerts',
  'Do-Not-Bet Heavy Chalk List'
]
const WORLD_CUP_HIGH_PRIORITY_SCANS = [
  ...
  'Brazil group winner',
  'Senegal/Norway top-2 or qualify',
  'USA/Mexico/Canada home-field advance markets'
]
...
function normalizeRunLeague(value = '') {
  ...
  const league = raw.replace(/[_-]+/g, ' ').replace(/\s+/g, ' ')
  const aliases = {
    FOOTBALL: 'SOCCER',
    FUTBOL: 'SOCCER',
  ...
  if (aliases[league]) return aliases[league]
...
function isWorldCupContext(value = '') {
  return /\b(world[_ -]?cup|fifa|group [a-l]\b|group winner|top[- ]?2|qualif(?:y|y from group|ication)|bracket collision|spain|argentina|france|norway|senegal|usa|mexico|canada|soccer|football)\b/i
    .test(String(value || ''))
}

function frameworkPrompt(framework = [], context = {}) {
  const league = String(context.league || context.sport || '').toLowerCase()
  const worldCupMode = isWorldCupContext(`${context.league || ''} ${context.sport || ''}`)
  ...
  if (ultraFastMode) {
    return [
      ...
      `- Apply ${WORLD_CUP_PATH_LEVERAGE_FILE} before grading World Cup futures, group winner, qualify, top-2, and live group-position bets.`,
      `- Apply ${POST_JUNE8_ADJUSTMENTS_FILE}: risk-adjust MLB full-game totals before grade assignment, cap extreme run environments at B-/Watchlist without 4 run-suppression confirmations, and reject fragile MLB totals from parlays.`,
      '- World Cup conditional Cup Win % by group finish is path leverage, not raw title probability; official soccer plays need sportsbook price, no-bet cutoff, market type, and path edge explanation.',
  ...
  if (fastMode) {
    ...
    addSnippet(WORLD_CUP_PATH_LEVERAGE_FILE, worldCupMode ? 3600 : 1400)
    addSnippet(POST_JUNE8_ADJUSTMENTS_MD_FILE, 1700)
    addSnippet(POST_JUNE8_ADJUSTMENTS_FILE, 2200)
  ...
  const relevant = new Set(['README.md', 'a-grade-hunt.md', A_GRADE_HUNT_RULES_FILE, WORLD_CUP_PATH_LEVERAGE_FILE, POST_JUNE8_ADJUSTMENTS_MD_FILE, POST_JUNE8_ADJUSTMENTS_FILE, 'bankroll.md', 'data-integrity.md', 'closing-odds.md'])
...
function candidateText(candidate = {}) {
  return [
    textFrom(candidate, ['Sport', 'sport']),
    ...
    textFrom(candidate, ['Tags', 'tags']),
    textFrom(candidate, ['Park/Weather Risk', 'parkWeatherRisk']),
    textFrom(candidate, ['Blow-Up Risk', 'blowUpRisk']),
    textFrom(candidate, ['Volatility Capped', 'volatilityCapped']),
...
function candidateScore(candidate = {}) {
  const cached = numberValue(candidate._score, NaN)
  if (Number.isFinite(cached)) return cached
  const rank = numberValue(candidate.Rank ?? candidate.rank, NaN)
  if (Number.isFinite(rank) && rank > 0) return 1000 - rank
  const risk = postJune8RiskProfile(candidate)
  const wnbaGap = wnbaModelGap(candidate)
  const riskAdjustment =
    (isRoleStableProp(candidate) ? 8 : 0) +
    (wnbaGap >= 5 && wnbaMarketSupport(candidate) ? 8 : 0) +
    (wnbaGap >= 4 && wnbaRoleConfirmation(candidate) ? 5 : 0) -
    (fragileHighVolatilityMlbTotal(candidate) ? 25 : 0) -
    (risk.capped ? 12 : 0)
  return gradeScore(textFrom(candidate, ['Grade', 'grade'])) +
    ...
    riskAdjustment +
    worldCupScoreAdjustment(candidate)
}
...
function candidateType(candidate = {}) {
  ...
  if (hasWord(/\b(player prop|prop|points|rebounds|assists|\bpra\b|threes|strikeouts|shots|sog|yards|hits|total bases|\btb\b|hrr|home runs?)\b/i, value) ||
...
function positiveEdge(candidate = {}) {
  ...
  return ['A+', 'A', 'A-', 'B+', 'B', 'B-', 'C+', 'C'].includes(normalizeGrade(textFrom(candidate, ['Grade', 'grade'])))
}
...
function aGradeText(candidate = {}) {
  return [
    ...
    textFrom(candidate, ['Park/Weather Risk']),
    textFrom(candidate, ['Blow-Up Risk']),
    textFrom(candidate, ['Volatility Capped']),
    textFrom(candidate, ['Tags']),
    textFrom(candidate, ['Source Verification', 'sourceVerification']),
    textFrom(candidate, ['Market Notes', 'marketNotes']),
    textFrom(candidate, ['Injury Notes', 'injuryNotes']),
    textFrom(candidate, ['Full Analysis', 'fullAnalysis']),
...
function tagList(candidate = {}) {
  const raw = candidate.Tags ?? candidate.tags ?? []
...
function isMlbCandidate(candidate = {}) {
  return hasWord(/\b(mlb|major league baseball|baseball)\b/i, [
    textFrom(candidate, ['Sport', 'sport']),
    textFrom(candidate, ['League', 'league'])
  ].join(' '))
}

function isWnbaCandidate(candidate = {}) {
  return hasWord(/\bwnba\b/i, [
    textFrom(candidate, ['Sport', 'sport']),
    textFrom(candidate, ['League', 'league'])
  ].join(' '))
}

function isFirstFive(candidate = {}) {
  return hasWord(/\b(first[- ]?5|first five|1st[- ]?5|\bf5\b)\b/i, candidateText(candidate))
}

function isTeamTotal(candidate = {}) {
  return hasWord(/\b(team total|team runs|\btt\b)\b/i, candidateText(candidate))
}

function totalLine(candidate = {}) {
  const direct = numberValue(textFrom(candidate, ['Line', 'line', 'Best Number', 'bestNumber', 'Total', 'total']), NaN)
  if (Number.isFinite(direct) && direct > 0 && direct < 30) return direct
  const match = candidateText(candidate).match(/\b(?:over|under|total)\s+([0-9]+(?:\.[0-9]+)?)/i)
  if (!match) return NaN
  const parsed = Number(match[1])
  return Number.isFinite(parsed) ? parsed : NaN
}

function isMlbFullGameTotal(candidate = {}) {
  return isMlbCandidate(candidate) &&
    marketFamily(candidate) === 'total' &&
    !isFirstFive(candidate) &&
    !isTeamTotal(candidate) &&
    candidateType(candidate) !== 'prop'
}

function runSuppressionConfirmations(candidate = {}) {
  const explicit = numberValue(textFrom(candidate, ['Run Suppression Confirmations', 'runSuppressionConfirmations']), NaN)
  if (Number.isFinite(explicit)) return Math.max(0, Math.floor(explicit))
  const value = candidateText(candidate)
  const checks = [
    /\b(starters?|probable pitchers?).{0,60}\b(confirmed|length|workload|innings|deep)\b/i,
    /\b(bullpens?).{0,60}\b(rested|above average|fresh|available|stable)\b/i,
    /\b(weather|park|roof|wind).{0,70}\b(neutral|pitcher[- ]friendly|suppresses|dead air|roof closed)\b/i,
    /\b(umpire|zone).{0,60}\b(not over|under[- ]leaning|pitcher[- ]friendly|wide zone|neutral)\b/i,
    /\b(market|line movement|steam).{0,70}\b(not moving over|not against under|under support|buyback|stable)\b/i,
    /\b(lineup|power|contact|bats).{0,80}\b(no major risk|suppressed|weak|resting|low power|cold)\b/i
  ]
  return checks.reduce((count, pattern) => count + (pattern.test(value) ? 1 : 0), 0)
}

function mlbExtremeRunEnvironmentFlags(candidate = {}) {
  if (!isMlbFullGameTotal(candidate)) return []
  const value = candidateText(candidate)
  const line = totalLine(candidate)
  const flags = []
  if (hasWord(/\blas vegas ballpark\b/i, value)) flags.push('Las Vegas Ballpark')
  if (hasWord(/\b(minor[- ]league|neutral[- ]site|pcl[- ]style|hitter[- ]friendly park|hitter park)\b/i, value)) flags.push('minor-league / neutral-site hitter park')
  if (hasWord(/\b(heat|hot weather|altitude|boosting carry|carry boost|wind out|thin air|humidity)\b/i, value)) flags.push('heat or altitude boosting carry')
  if (Number.isFinite(line) && line >= 10.5) flags.push('total of 10.5 or higher')
  if (hasWord(/\b(homer[- ]prone|low ground[- ]ball|fly[- ]ball|barrel risk|hard[- ]hit risk)\b/i, value)) flags.push('homer-prone starter')
  if (hasWord(/\b(tired bullpen|fatigued bullpen|bad bullpen|below[- ]average bullpen|bullpen fatigue|taxed bullpen)\b/i, value)) flags.push('tired/bad bullpen')
  if (hasWord(/\b(market moving over|moving over|over steam|line moving toward over|steaming over)\b/i, value)) flags.push('market moving Over')
  return Array.from(new Set(flags))
}

function explicitBlowUpRisk(candidate = {}) {
  const risk = String(textFrom(candidate, ['Blow-Up Risk', 'blowUpRisk']) || '').trim()
  return /^(low|medium|high)$/i.test(risk) ? risk[0].toUpperCase() + risk.slice(1).toLowerCase() : ''
}

function postJune8RiskProfile(candidate = {}) {
  const flags = mlbExtremeRunEnvironmentFlags(candidate)
  const confirmations = runSuppressionConfirmations(candidate)
  const explicitRisk = explicitBlowUpRisk(candidate)
  const highRisk = explicitRisk === 'High' ||
    flags.length >= 3 ||
    (flags.includes('Las Vegas Ballpark') && flags.length >= 2) ||
    (flags.includes('total of 10.5 or higher') && flags.length >= 2)
  const blowUpRisk = explicitRisk || (highRisk ? 'High' : (flags.length ? 'Medium' : 'Low'))
  const capped = isMlbFullGameTotal(candidate) && flags.length > 0 && confirmations < 4
  const parkWeatherConflict = flags.some(flag => /park|weather|heat|altitude|vegas|neutral/i.test(flag))
  const parkWeatherRisk = textFrom(candidate, ['Park/Weather Risk', 'parkWeatherRisk']) ||
    (parkWeatherConflict ? `${blowUpRisk}: ${flags.filter(flag => /park|weather|heat|altitude|vegas|neutral/i.test(flag)).join(', ')}` : 'Low')
  return {
    isMlbFullGameTotal: isMlbFullGameTotal(candidate),
    flags,
    confirmations,
    blowUpRisk,
    capped,
...
function capGradeAtBMinus(grade = '') {
  const normalized = normalizeGrade(grade)
  if (!normalized) return 'B-'
  return gradeScore(normalized) > gradeScore('B-') ? 'B-' : normalized
}

function fragileHighVolatilityMlbTotal(candidate = {}) {
  const risk = postJune8RiskProfile(candidate)
  return risk.isMlbFullGameTotal && (risk.capped || ['Medium', 'High'].includes(risk.blowUpRisk))
}

function postJune8Watchlist(candidate = {}) {
  const risk = postJune8RiskProfile(candidate)
  return risk.capped && risk.blowUpRisk === 'High'
}

function isSoccerCandidate(candidate = {}) {
  return isWorldCupContext(candidateText(candidate)) ||
    hasWord(/\b(soccer|football|fifa|world cup)\b/i, [
      textFrom(candidate, ['Sport', 'sport']),
      textFrom(candidate, ['League', 'league'])
    ].join(' '))
}

function hasPathEdgeExplanation(candidate = {}) {
  return hasWord(/\b(path edge|path leverage|group finish|group winner|top[- ]?2|qualif(?:y|y from group|ication)|bracket|collision|host[- ]field|home[- ]field|conditional cup win|knockout path|advance market|live group[- ]position)\b/i, candidateText(candidate))
}

function worldCupMissingOfficialFields(candidate = {}) {
  if (!isSoccerCandidate(candidate)) return []
  const missing = []
  if (!exactAmericanOdds(textFrom(candidate, ['Odds', 'odds']))) missing.push('sportsbook price')
  if (!verifiedSportsbook(textFrom(candidate, ['Sportsbook', 'sportsbook']))) missing.push('sportsbook')
  if (!textFrom(candidate, ['No Bet Cutoff', 'noBetCutoff'])) missing.push('no-bet cutoff')
  if (!textFrom(candidate, ['Bet Type', 'betType', 'Market', 'market'])) missing.push('market type')
  if (!hasPathEdgeExplanation(candidate)) missing.push('path edge explanation')
  return missing
}

function worldCupMarketPriority(candidate = {}) {
  const value = candidateText(candidate)
  if (hasWord(/\b(group winner|win group|to win group|group [a-l] winner)\b/i, value)) return 'group_winner'
  if (hasWord(/\b(top[- ]?2|top two|qualif(?:y|y from group|ication)|to advance from group)\b/i, value)) return 'qualify_top2'
  if (hasWord(/\b(live group[- ]position|live group position|group position|finish(?:ing)? 1st|finish(?:ing)? 2nd|finish(?:ing)? first|finish(?:ing)? second)\b/i, value)) return 'live_group_position'
  if (hasWord(/\b(to advance|knockout|round of 32|round of 16|quarterfinal|semifinal)\b/i, value)) return 'advance'
  if (hasWord(/\b(outright|future|to win world cup|world cup winner|title)\b/i, value)) return 'outright_future'
  if (hasWord(/\b(moneyline|\bml\b)\b/i, value)) return 'moneyline'
  return ''
}

function isHeavyFriendlyChalk(candidate = {}) {
  const odds = numberValue(textFrom(candidate, ['Odds', 'odds']), NaN)
  return isSoccerCandidate(candidate) &&
    worldCupMarketPriority(candidate) === 'moneyline' &&
    Number.isFinite(odds) &&
    odds <= -220 &&
    hasWord(/\bfriendly|friendlies|exhibition\b/i, candidateText(candidate))
}

function worldCupIndependentPriceEdge(candidate = {}) {
  const edge = candidateEdge(candidate)
  return Number.isFinite(edge) && edge >= 5 && hasIndependentEdge(candidate) && hasPathEdgeExplanation(candidate)
}

function worldCupScoreAdjustment(candidate = {}) {
  if (!isSoccerCandidate(candidate)) return 0
  const priority = worldCupMarketPriority(candidate)
  let score = 0
  if (priority === 'group_winner') score += 12
  if (priority === 'qualify_top2') score += 9
  if (priority === 'live_group_position') score += 9
  if (priority === 'advance') score += 5
  if (priority === 'outright_future') score -= 10
  if (isHeavyFriendlyChalk(candidate)) score -= 30
  if (hasPathEdgeExplanation(candidate)) score += 5
  return score
}

function applyWorldCupPathLeverage(candidate = {}) {
  if (!isSoccerCandidate(candidate)) return candidate
  const next = { ...candidate }
  const tags = new Set(tagList(next))
  const priority = worldCupMarketPriority(next)
  const missing = worldCupMissingOfficialFields(next)

  if (priority === 'group_winner' || priority === 'qualify_top2' || priority === 'live_group_position') tags.add('World Cup Path Leverage')
  if (hasWord(/\b(spain|group h|argentina|group j|collision)\b/i, candidateText(next))) tags.add('Bracket Collision Alert')
  if (hasWord(/\b(france|norway|senegal|group i)\b/i, candidateText(next))) tags.add('Group I Monitor')
  if (hasWord(/\b(usa|united states|mexico|canada|host[- ]field|home[- ]field)\b/i, candidateText(next))) tags.add('Host-Field Adjustment')

  if (priority === 'outright_future' && !hasPathEdgeExplanation(next)) {
    next.Grade = capGradeAtBMinus(next.Grade)
    next['Market Notes'] = appendNote(next['Market Notes'], 'World Cup Path Leverage cap: outright futures need path and price improvement before official upgrade.')
  }

  if (isHeavyFriendlyChalk(next) && !worldCupIndependentPriceEdge(next)) {
    tags.add('Do-Not-Bet Heavy Chalk')
    next.Grade = capGradeAtBMinus(next.Grade)
    next['Release Gate'] = 'Watchlist'
    next['Market Notes'] = appendNote(next['Market Notes'], 'World Cup Path Leverage cap: heavy friendly moneyline stays pass/watchlist unless independent price edge is proven.')
    next['A Grade Gate Result'] = appendNote(next['A Grade Gate Result'], 'World Cup gate: heavy friendly chalk cannot be official without independent price edge.')
  }

  if (missing.length) {
    next['Release Gate'] = 'Watchlist'
    next['Market Notes'] = appendNote(next['Market Notes'], `World Cup official-play gate missing: ${missing.join(', ')}.`)
  }

  next.Tags = tagString(tags)
  next._worldCupMarketPriority = priority
  next._worldCupMissingOfficialFields = missing
  return next
}

function isRoleStableProp(candidate = {}) {
  if (candidateType(candidate) !== 'prop') return false
  return hasWord(/\b(role[- ]stable|stable role|confirmed role|confirmed lineup|lineup confirmed|minutes secure|usage secure|starter confirmed|batting order confirmed)\b/i, candidateText(candidate))
}

function wnbaModelGap(candidate = {}) {
  if (!isWnbaCandidate(candidate) || marketFamily(candidate) !== 'spread') return NaN
  const explicit = numberValue(textFrom(candidate, ['Model Gap', 'Projection Gap', 'Point Gap', 'Line Cushion']), NaN)
  if (Number.isFinite(explicit)) return Math.abs(explicit)
  const value = candidateText(candidate)
  const direct = value.match(/\b([0-9]+(?:\.[0-9]+)?)\+?\s*(?:point|pt)[a-z]*\s+(?:model|projection|line)?\s*(?:gap|edge|cushion)\b/i)
  if (direct) return Number(direct[1])
  const reversed = value.match(/\b(?:model|projection|line)\s*(?:gap|edge|cushion).{0,20}\b([0-9]+(?:\.[0-9]+)?)\+?\s*(?:point|pt)/i)
  return reversed ? Number(reversed[1]) : NaN
}

function wnbaRoleConfirmation(candidate = {}) {
  return hasWord(/\b(injury|rotation|role|starter|minutes|availability).{0,70}\b(confirmed|clear|stable|available|ruled out|active)\b/i, candidateText(candidate))
}

function wnbaMarketSupport(candidate = {}) {
  return hasWord(/\b(market support|line cushion|not moving against|market not moving against|buyback|steam|closing value|inside cutoff)\b/i, candidateText(candidate))
}

function applyPostJune8Adjustments(candidate = {}) {
  const next = { ...candidate }
  const tags = new Set(tagList(next))
  const wnbaGap = wnbaModelGap(next)
  const roleStable = isRoleStableProp(next)
  const risk = postJune8RiskProfile(next)

  if (roleStable) tags.add('Role-Stable Prop')

  if (Number.isFinite(wnbaGap) && wnbaGap >= 4 && wnbaRoleConfirmation(next)) {
    tags.add('B+ Near-A')
    if (!['PASS', 'A+', 'A'].includes(normalizeGrade(next.Grade)) && gradeScore(next.Grade) < gradeScore('B+')) {
      next.Grade = 'B+'
    }
    next['Market Notes'] = appendNote(next['Market Notes'], `Post-June 8 WNBA model-gap upgrade: ${wnbaGap}-point gap with role/injury confirmation is B+ eligible.`)
  }

  if (Number.isFinite(wnbaGap) && wnbaGap >= 5 && wnbaMarketSupport(next)) {
    tags.add('A-Hunt Candidate')
    next['A-Hunt Source Notes'] = appendNote(next['A-Hunt Source Notes'], `WNBA ${wnbaGap}-point model gap with market support is A-candidate eligible, subject to the normal A-grade gate.`)
  }

  if (risk.isMlbFullGameTotal) {
    next['Park/Weather Risk'] = risk.parkWeatherRisk
    next['Blow-Up Risk'] = risk.blowUpRisk
    next['Volatility Capped'] = risk.capped
      ? `Yes: ${risk.flags.join(', ')}; ${risk.confirmations}/4 run-suppression confirmations.`
      : `No: ${risk.confirmations}/4 run-suppression confirmations.`

    if (risk.flags.length) tags.add('Extreme Run Environment')
    if (risk.parkWeatherConflict) tags.add('Park/Weather Conflict')

    if (risk.capped) {
      tags.add('Volatile Total Cap')
      next.Grade = capGradeAtBMinus(next.Grade)
      next['Market Notes'] = appendNote(next['Market Notes'], `Post-June 8 volatile MLB total cap applied: ${risk.flags.join(', ')} with only ${risk.confirmations}/4 run-suppression confirmations.`)
      next['A Grade Gate Result'] = appendNote(next['A Grade Gate Result'], 'Post-June 8 cap: this MLB full-game total cannot clear above B- until 4 run-suppression confirmations exist.')
      if (risk.blowUpRisk === 'High') next['Release Gate'] = 'Watchlist'
    }
  }

  if (['A+', 'A'].includes(normalizeGrade(next.Grade))) tags.add('A-Hunt Candidate')
  if (normalizeGrade(next.Grade) === 'B+') tags.add('B+ Near-A')
  next.Tags = tagString(tags)
  next._postJune8Risk = risk
  return next
}
...
function normalizeReleaseGate(candidate = {}) {
  ...
  if (postJune8Watchlist(candidate)) return 'Watchlist'
  if (isSoccerCandidate(candidate) && worldCupMissingOfficialFields(candidate).length) return 'Watchlist'
  if (isHeavyFriendlyChalk(candidate) && !worldCupIndependentPriceEdge(candidate)) return 'Watchlist'
  if (hasVerifiedReviewPrice(candidate)) return 'Review Eligible'
...
function decorateCandidate(candidate = {}) {
  const preGatedCandidate = applyWorldCupPathLeverage(applyPostJune8Adjustments(candidate))
  const aGatedCandidate = applyAGradeGate(preGatedCandidate)
  const postJune8Candidate = applyWorldCupPathLeverage(applyPostJune8Adjustments(aGatedCandidate))
  const normalizedGate = normalizeReleaseGate(postJune8Candidate)
  const confidence = sourceConfidence(postJune8Candidate)
  const manualRequired = manualConfirmationRequired(postJune8Candidate)
  return {
    ...postJune8Candidate,
    'Eligible Routes': eligibleRoutes(postJune8Candidate),
    'Original Release Gate': textFrom(postJune8Candidate, ['Release Gate', 'releaseGate']) || '',
    'Release Gate': normalizedGate,
    sourceConfidence: confidence,
    missingFields: missingCandidateFields(postJune8Candidate),
    manualConfirmationRequired: manualRequired,
    _type: candidateType(postJune8Candidate),
    _score: candidateScore(postJune8Candidate)
  }
}
...
function lottoLegEligible(candidate = {}, allowReview = false) {
  ...
  if (!routeGateOk(candidate)) return false
  if (hasNegativeEdge(candidate)) return false
  if (fragileHighVolatilityMlbTotal(candidate)) return false
...
function appearsSampleCard(card = {}) {
  return ['Game', 'Pick', 'Source Verification', 'Writeup', 'Short Take', 'Why This Play', 'Matchup Edge', 'Projection Edge', 'Key Metrics', 'Risk', 'Final Take', 'Full Analysis']
...
function aiJsonSchema(maxRawItems = 4) {
  const candidateProperties = {
    ...
    'Short Take': { type: 'string' },
    'Why This Play': { type: 'string' },
    'Matchup Edge': { type: 'string' },
    'Projection Edge': { type: 'string' },
    'Key Metrics': { type: 'string' },
    Risk: { type: 'string' },
    'Final Take': { type: 'string' },
    ...
    'Park/Weather Risk': { type: 'string' },
    'Blow-Up Risk': { type: 'string' },
    'Volatility Capped': { type: 'string' },
    Tags: { type: 'string' },
...
function openAiPropsRequestBody({ model, date, sport, league, routing, retry = false }) {
  const prompt = [
    ...
    'Tag role-stable props with Role-Stable Prop and A-Hunt Candidate or B+ Near-A when supported; role-stable props can outrank volatile MLB full-game totals.',
...
function openAiRequestBody({ model, date, sport, league, mode, candidatePoolSize, access, framework, routing, propsSearch = false, retry = false, fastMode = true, ultraFastMode = false }) {
  ...
  const worldCupMode = isWorldCupContext(`${sport || ''} ${league || ''}`)
      ...
      content: [{
        ...
        text: [
          ...
          propsSearch ? frameworkPrompt(framework, {
            ...
            worldCupMode
              ? `- ${league || sport || 'World Cup'} board only: group winner, top-2/qualify, live group-position, knockout advance, path-adjusted futures, and price-backed match markets.`
            ...
            worldCupMode ? '- World Cup Path Leverage Mode is on. Conditional Cup Win % by group finish is path leverage, not raw title probability.' : '',
            worldCupMode ? '- Prioritize group winner, top-2, qualify, and live group-position markets before outright futures.' : '',
            worldCupMode ? '- Track bracket collision risk for Spain Group H and Argentina Group J, monitor Group I for France/Norway/Senegal, and apply host-field adjustments for USA, Mexico, and Canada.' : '',
            worldCupMode ? '- Heavy friendly moneylines are Pass/Watchlist by default unless independent price edge is proven.' : '',
            worldCupMode ? '- Do not create official soccer plays without sportsbook price, no-bet cutoff, market type, and path edge explanation.' : '',
            ...
            '- Apply the Post-June 8 adjustment pack: risk-adjust MLB full-game totals before grade assignment, cap extreme run environments at B-/Watchlist without 4 run-suppression confirmations, and never use fragile high-volatility MLB totals as parlay legs.',
            '- Every MLB full-game total must include Park/Weather Risk, Blow-Up Risk (Low/Medium/High), Volatility Capped, and Tags.',
            '- WNBA spread model gaps may upgrade more aggressively: 4+ point gap plus injury/rotation confirmation is B+ eligible; 5+ point gap plus market support is A-candidate eligible.',
          ...
          propsSearch
            ...
            : (worldCupMode
              ? '- Use web search once to gather current sportsbook prices for World Cup group winner, top-2/qualify, live group-position, advance, futures, and match markets plus bracket/path context.'
          ...
          worldCupMode ? '- Build the required output sections from sourced candidates: World Cup Path Watchlist, Group Winner Edge Board, Live Group-Position Triggers, Host-Field Adjustment Notes, Bracket Collision Alerts, and Do-Not-Bet Heavy Chalk List.' : '',
          '- Before final grading, apply Post-June 8 risk adjustment: projection edge alone cannot promote volatile MLB full-game totals; prefer WNBA model-gap edges, first-5/team totals, and role-stable props over fragile MLB totals.',
          propsSearch
            ? '- Treat this as a props-only follow-up after the core board JSON already parsed.'
            : (worldCupMode
              ? '- Treat source acquisition as soccer/World Cup board-only: group markets, qualify/top-2, futures, live group-position, advance, and price-backed match markets. Do not source unrelated props.'
          ...
          '- Each candidate must include only schema fields: Sport, League, Game, Pick, Bet Type, Odds, Sportsbook, Line, Best Number, No Bet Cutoff, Grade, Confidence, Implied Probability, Estimated True Probability, EV Edge, Market Notes, Injury Notes, Source Verification, Short Take, Why This Play, Matchup Edge, Projection Edge, Key Metrics, Risk, Final Take, A Grade Gate Result, A Grade Evidence Count, Market Misprice Reason, Unresolved Conflict, A-Hunt Source Notes, Park/Weather Risk, Blow-Up Risk, Volatility Capped, Tags, sourceConfidence, manualConfirmationRequired, summary.',
          '- Write Short Take / Why This Play / Matchup Edge / Projection Edge / Key Metrics / Risk / Final Take like a real handicapper explaining the play to a subscriber. Name the actual player/team, opponent, line, projection gap, best number, cutoff, and what can beat the bet. Do not use generic phrases like "model edge is present", "trend data supports this play", or "positive EV based on matchup indicators."',
...
function worldCupSectionItem(candidate = {}) {
  const pick = textFrom(candidate, ['Pick', 'pick']) || 'World Cup candidate'
  return {
    Candidate: [textFrom(candidate, ['Game', 'game']), pick].filter(Boolean).join(' - ') || pick,
    Market: textFrom(candidate, ['Bet Type', 'betType', 'Market', 'market']),
    Pick: pick,
    ...
    'Path Edge Explanation': textFrom(candidate, ['Market Notes', 'marketNotes']) ||
      textFrom(candidate, ['A-Hunt Source Notes']) ||
      textFrom(candidate, ['summary']),
    'Release Gate': textFrom(candidate, ['Release Gate']),
    Tags: textFrom(candidate, ['Tags'])
  }
}

function emptyWorldCupOutputSections() {
  return {
    worldCupPathWatchlist: [],
    'World Cup Path Watchlist': [],
    groupWinnerEdgeBoard: [],
    'Group Winner Edge Board': [],
    liveGroupPositionTriggers: [],
    'Live Group-Position Triggers': [],
    hostFieldAdjustmentNotes: [],
    'Host-Field Adjustment Notes': [],
    bracketCollisionAlerts: [],
    'Bracket Collision Alerts': [],
    doNotBetHeavyChalkList: [],
    'Do-Not-Bet Heavy Chalk List': []
  }
}

function buildWorldCupOutputSections(candidates = []) {
  const worldCup = sortCandidates(candidates.filter(isSoccerCandidate))
  const section = filter => worldCup.filter(filter).slice(0, 12).map(worldCupSectionItem)
  const watchlist = section(candidate =>
    hasWord(/\b(watchlist|path leverage|group winner|top[- ]?2|qualif|host[- ]field|bracket|collision|group [hij])\b/i, candidateText(candidate)) ||
    hasWord(/\bwatchlist\b/i, textFrom(candidate, ['Release Gate']))
  )
  const groupWinners = section(candidate => worldCupMarketPriority(candidate) === 'group_winner')
  const liveTriggers = section(candidate => worldCupMarketPriority(candidate) === 'live_group_position')
  const hostNotes = section(candidate => hasWord(/\b(usa|united states|mexico|canada|host[- ]field|home[- ]field)\b/i, candidateText(candidate)))
  const collisionAlerts = section(candidate => hasWord(/\b(spain|group h|argentina|group j|collision)\b/i, candidateText(candidate)))
  const heavyChalk = section(candidate => isHeavyFriendlyChalk(candidate) || hasWord(/\b(do[- ]not[- ]bet heavy chalk|heavy friendly|heavy chalk)\b/i, candidateText(candidate)))
  return {
    worldCupPathWatchlist: watchlist,
    'World Cup Path Watchlist': watchlist,
    groupWinnerEdgeBoard: groupWinners,
    'Group Winner Edge Board': groupWinners,
    liveGroupPositionTriggers: liveTriggers,
    'Live Group-Position Triggers': liveTriggers,
    hostFieldAdjustmentNotes: hostNotes,
    'Host-Field Adjustment Notes': hostNotes,
    bracketCollisionAlerts: collisionAlerts,
    'Bracket Collision Alerts': collisionAlerts,
    doNotBetHeavyChalkList: heavyChalk,
    'Do-Not-Bet Heavy Chalk List': heavyChalk
  }
}
...
function normalizeAiCard(card = {}, options = {}) {
  ...
  const normalized = {
    ...
    'Short Take': card['Short Take'] || '',
    'Why This Play': card['Why This Play'] || '',
    'Matchup Edge': card['Matchup Edge'] || '',
    'Projection Edge': card['Projection Edge'] || '',
    'Key Metrics': card['Key Metrics'] || '',
    Risk: card.Risk || card['Risk Notes'] || '',
    'Final Take': card['Final Take'] || '',
    ...
    'Park/Weather Risk': card['Park/Weather Risk'] || '',
    'Blow-Up Risk': card['Blow-Up Risk'] || '',
    'Volatility Capped': card['Volatility Capped'] || '',
    Tags: card.Tags || '',
    Player: card.Player || '',
    Team: card.Team || '',
...
function normalizeCandidate(candidate = {}, options = {}) {
  const baseCard = {
    ...
    'Short Take': candidate['Short Take'] || candidate.summary || '',
    'Why This Play': candidate['Why This Play'] || candidate.summary || '',
    'Matchup Edge': candidate['Matchup Edge'] || '',
    'Projection Edge': candidate['Projection Edge'] || '',
    'Key Metrics': candidate['Key Metrics'] || '',
    Risk: candidate.Risk || candidate['Risk Notes'] || candidate['Risk/Variance Note'] || '',
    'Final Take': candidate['Final Take'] || '',
    ...
    'Park/Weather Risk': candidate['Park/Weather Risk'] || '',
    'Blow-Up Risk': candidate['Blow-Up Risk'] || '',
    'Volatility Capped': candidate['Volatility Capped'] || '',
    Tags: candidate.Tags || '',
...
function heldReviewCard(candidate = {}, options = {}) {
  ...
  const baseCard = {
    ...
    'Short Take': textFrom(candidate, ['Short Take', 'shortTake', 'summary']),
    'Why This Play': textFrom(candidate, ['Why This Play', 'whyThisPlay', 'summary']),
    'Matchup Edge': textFrom(candidate, ['Matchup Edge', 'matchupEdge']),
    'Projection Edge': textFrom(candidate, ['Projection Edge', 'projectionEdge']),
    'Key Metrics': textFrom(candidate, ['Key Metrics', 'keyMetrics']),
    Risk: textFrom(candidate, ['Risk', 'risk', 'Risk Notes']),
    'Final Take': textFrom(candidate, ['Final Take', 'finalTake']),
    ...
    'Park/Weather Risk': textFrom(candidate, ['Park/Weather Risk']),
    'Blow-Up Risk': textFrom(candidate, ['Blow-Up Risk']),
    'Volatility Capped': textFrom(candidate, ['Volatility Capped']),
    Tags: textFrom(candidate, ['Tags']),
...
function makeParlayCard({ legs, legCount, parlayType, date, mode, access, sport, league, reviewOnly = false }) {
  ...
  const legTags = tagString(legs.flatMap(leg => tagList(leg)))
  const legBlowUpRisks = legs.map(leg => textFrom(leg, ['Blow-Up Risk'])).filter(Boolean)
  const legParkWeatherRisks = legs.map(leg => textFrom(leg, ['Park/Weather Risk'])).filter(Boolean)
  const legVolatilityCaps = legs.map(leg => textFrom(leg, ['Volatility Capped'])).filter(Boolean)
  ...
  const shortTake = `${legCount}-leg ${parlayType} built from verified pool legs; confirm every leg still sits inside its playable number.`
  const whyThisPlay = `${parlayType} only belongs on the card because the legs already qualified on their own, then combine into a plus-price payout without forcing a weak add-on.`
  const matchupEdge = `The edge comes from the exact leg mix: ${legList(legs)}. Each leg still needs its matchup, injury, lineup, and market context to match the original handicap.`
  const projectionEdge = odds
    ? `Combined price is estimated at ${odds}${sportsbooks.length ? ` from ${sportsbooks.join(', ')}` : ''}; play it only if the book price stays close to the listed leg numbers.`
    : 'Projected parlay value depends on confirming the final combined price at the book before release.'
  const keyMetrics = [
    `Leg count: ${legCount}`,
    avgConfidence ? `Average confidence: ${avgConfidence}` : '',
    aEvidenceCounts.length ? `A-hunt evidence count high: ${Math.max(...aEvidenceCounts)}` : '',
    legTags ? `Tags: ${legTags}` : ''
  ].filter(Boolean).join('. ')
  const risk = reviewOnly
    ? 'The risk is number drift before release; keep this held if the final combined number moves materially away from the listed price.'
    : 'The risk is normal parlay variance: one correlated game script, stale price, late scratch, or lineup shift can turn a strong leg into the losing leg.'
  const finalTake = reviewOnly ? 'Watchlist/live-only until the combined price confirms.' : `${parlayType} lotto play at the confirmed book price.`
  ...
  return normalizeAiCard({
    ...
    'Park/Weather Risk': legParkWeatherRisks.join(' | '),
    'Blow-Up Risk': legBlowUpRisks.join(' | ') || 'Low',
    'Volatility Capped': legVolatilityCaps.join(' | '),
    Tags: legTags,
...
function validateCard(card = {}) {
  ...
  const heldOrManual = hasWord(/\b(held|manual review|watchlist)\b/i, `${card['Release Status'] || ''} ${card.Category || ''} ${card.Status || ''}`)
  if (isSoccerCandidate(card) && !heldOrManual) {
    missing.push(...worldCupMissingOfficialFields(card))
  }
...
function routeCandidatePool(rawCandidates = [], options = {}) {
  ...
  const worldCupOutputSections = buildWorldCupOutputSections(decorated)
  ...
  const fragileMlbTotalParlayRejects = routeBase.filter(candidate =>
    !['longshot', 'prop'].includes(candidate._type) &&
    routeGateOk(candidate) &&
    fragileHighVolatilityMlbTotal(candidate)
  )
  ...
  if (fragileMlbTotalParlayRejects.length) {
    warnings.push(`${fragileMlbTotalParlayRejects.length} fragile MLB total candidate(s) rejected from lotto parlay construction by Post-June 8 rules.`)
    passes.push(...fragileMlbTotalParlayRejects.map(candidate => ({
      Candidate: textFrom(candidate, ['Pick', 'pick']) || 'MLB total',
      Reason: 'Rejected from lotto parlay construction by Post-June 8 fragile MLB total rule.',
      'Source Verification': textFrom(candidate, ['Source Verification', 'sourceVerification']) || 'Source not verified.'
    })))
  }
  const routeOutputs = {
  ...
  const routingSummary = {
    ...
    postJune8Fields: POST_JUNE8_FIELDS,
    postJune8Tags: POST_JUNE8_TAGS,
    worldCupPathLeverageFile: WORLD_CUP_PATH_LEVERAGE_FILE,
    worldCupOutputSections: WORLD_CUP_OUTPUT_SECTION_NAMES,
    worldCupHighPriorityScans: WORLD_CUP_HIGH_PRIORITY_SCANS,
    fragileMlbTotalParlayRejects: fragileMlbTotalParlayRejects.length,
    quotas: {
