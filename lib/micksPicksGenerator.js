...
import { buildCustomerFacingAnalysis, sanitizeCustomerFacingAnalysis } from './customerFacingAnalysis.js'
...
function runLeagues(options = {}) {
  ...
  if (isAllSportsValue(league) && isAllSportsValue(sport)) return SUPPORTED_RUN_LEAGUES
  const requested = normalizeRunLeague(league) || normalizeRunLeague(sport)
  return requested ? [requested] : SUPPORTED_RUN_LEAGUES
}
...
function hasVerifiedReviewPrice(candidate = {}) {
  const missingFields = Array.isArray(candidate.missingFields)
    ? candidate.missingFields.filter(field => String(field || '').trim())
    : []
  return sourceConfidence(candidate) === 'high' &&
    exactAmericanOdds(textFrom(candidate, ['Odds', 'odds'])) &&
    verifiedSportsbook(textFrom(candidate, ['Sportsbook', 'sportsbook'])) &&
    textFrom(candidate, ['Line', 'line', 'Best Number', 'bestNumber']) !== '' &&
    Boolean(textFrom(candidate, ['Source Verification', 'sourceVerification'])) &&
    missingFields.length === 0 &&
    !manualConfirmationRequired(candidate) &&
      ...
      content: [{
        ...
        text: [
          ...
          propsSearch ? frameworkPrompt(framework, {
            ...
            '- Return at most 2 current board ideas. Exact book-priced candidates should include American odds like -110 or +125, a real sportsbook/book label, line, and source URL/text.',
...
async function callOpenAiEngineForRun({ options, date, mode, access, framework, routing }) {
  ...
  for (const league of leagues) {
    if (aggregate.ai.rawCandidatePool.length >= maxTotal) break
    try {
      ...
      aggregate.ai.rawCandidatePool.push(...tagCandidatesForLeague(rawCandidatesFromEngine(engine), league).slice(0, maxPerLeague))
...
function manualReviewCard(candidate = {}, options = {}) {
  ...
  const line = textFrom(candidate, ['Line', 'line'])
  ...
  return normalizeAiCard({
    ...
    'Archive Status': 'Active',
    Access: options.access === 'auto' ? 'VIP Review' : options.access,
    Writeup: summary || `Manual review required before release: ${missingText || 'market fields'}.`,
    'Market Notes': [
      summary,
      missingText ? `Manual entry needed for: ${missingText}.` : 'Manual confirmation required before release.'
    ].filter(Boolean).join(' '),
    'Injury Notes': textFrom(candidate, ['Injury Notes', 'injuryNotes']),
    'Source Verification': sourceVerification,
...
export async function generateMicksPicks(options = {}) {
  ...
  const manualReviewStatus = {
    requested: true,
    created: mode === 'review' ? manualReviewPool.length : 0,
    pending: manualReviewPool.length,
    reason: manualReviewPool.length
