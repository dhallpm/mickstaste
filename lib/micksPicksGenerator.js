...
async function callOpenAiEngine({ date, sport, league, mode, candidatePoolSize, access, framework, routing, propsSearch = false, allowRetry = false }) {
  ...
  try {
    for (let attempt = 0; attempt < attempts; attempt += 1) {
      if (attempt > 0 && !String(firstRawText ?? '').trim()) break
    ...
    if (!retryUsed && error?.statusCode) throw error
    throw buildJsonParseError({
      ...
      retryUsed,
      parseFailures,
      parseErrorType: retryUsed ? 'repair_exception' : (parseErrorType || 'invalid_json'),
