...
export function calculateClvFields(row = {}) {
  ...
  if (moneyline && !Number.isFinite(closingOdds) && Number.isFinite(closingNumber) && Math.abs(closingNumber) >= 100) {
    closingOdds = closingNumber
  }
  ...
  if (moneyline) {
    ...
    return {
      ...
      reason: 'moneyline needs Closing Odds'
    }
  }

  if (Number.isFinite(closingNumber) && Number.isFinite(postedNumber)) {
    if (Math.abs(closingNumber) >= 100 && Math.abs(postedNumber) < 50) {
      return {
        fields: {
          'CLV Result': 'Insufficient Data'
        },
        reason: 'closing odds appear to be entered in closing number field'
      }
    }
