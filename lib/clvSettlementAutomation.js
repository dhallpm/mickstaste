...
function isParlayRow(row = {}) {
  ...
  return /\bparlay\b|\blotto\b|\blegs?\b|\|/.test(value)
}
...
export function calculateSettlementFields(row = {}, now = new Date(), options = {}) {
  ...
  if (result === 'Win') {
    if (!Number.isFinite(odds)) {
      ...
      profitLoss = 'Profit Pending - Missing Odds'
      roi = ''
    } else {
  ...
  return {
    fields: {
      ...
      ROI: roi,
      'Settled At': now.toISOString(),
      ...settlementAuditFields(row, verification)
    },
    skipped: false,
    reason: result
  }
...
async function settleTable(key, config, date, now, options = {}) {
  ...
  for (const record of records) {
    ...
    if (!rawResult || /^pending$/i.test(rawResult)) {
      ...
      if (!canAttemptSourceSettlement(fields, settleAll)) {
        skipped.push({ id: record.id, date: recordDate, result: rawResult || '', reason: 'Pending or blank Result and no result source URL' })
        continue
      }

      const routed = await routeSettlementSources(fields, { ...options, urls: sourceUrls })
      if (routed.status !== 'verified' || !routed.result) {
        needsReview.push({ record, fields: needsReviewFields(routed, now), reason: routed.notes })
        continue
      }
      ...
      if (calculated.skipped) {
        skipped.push({ id: record.id, date: recordDate, result: routed.result, reason: calculated.reason })
        continue
      }
      matched.push({ record, fields: calculated.fields, reason: calculated.reason, verification: routed })
      continue
    }
    const calculated = calculateSettlementFields(fields, now)
    if (calculated.skipped) {
  ...
  const updates = [
    ...matched.map(item => ({ id: item.record.id, fields: item.fields })),
    ...needsReview.map(item => ({ id: item.record.id, fields: item.fields }))
  ]
...
export async function settleResults(options = {}) {
  ...
  for (const [key, config] of Object.entries(TABLE_CONFIGS)) {
    try {
      const result = await settleTable(key, config, date, now, { ...options, settleAll, settleEveryDate })
