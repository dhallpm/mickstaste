...
function directionFromText(value = '') {
  const normalized = normalizedLegText(value)
  if (/\bover\b/.test(normalized)) return 'over'
  if (/\bunder\b/.test(normalized)) return 'under'
  return ''
}

function thresholdFromText(value = '') {
  const match = text(value).match(/\b(?:over|under)\s*([+-]?\d+(?:\.\d+)?)/i)
  return match ? Number(match[1]) : NaN
}

function looksLikeShotsProp(value = '') {
  return /\b(?:shots?\s+on\s+goal|sog)\b/i.test(value)
}

function inferPlayerFromLeg(value = '') {
  return text(value)
    .replace(/\s*(?:-|\u2013)\s*/g, ' ')
    .replace(/\b(?:over|under)\b.*$/i, '')
    .replace(/\b\d+(?:\.\d+)?\s*\+\s*(?:shots?|sog|goals?|points?|rebounds?|assists?|strikeouts?|ks?)\b.*$/i, '')
    .trim()
}

function enrichPlayerFromSameDateParlays(row = {}, allRows = [], date = '') {
  if (text(firstValue(row, ['Player', 'Athlete', 'Player Name']))) return row
  const pick = firstValue(row, ['Pick', 'Selection', 'Play', 'Prop'])
  if (!looksLikeShotsProp(pick)) return row
  const rowDirection = directionFromText(pick)
  const rowThreshold = thresholdFromText(pick)
  for (const record of allRows) {
    const fields = recordFields(record)
    if (fields === row || rowDateKey(fields) !== date || !isParlayRow(fields)) continue
    for (const leg of splitPlusParlayLegs(fields)) {
      if (!looksLikeShotsProp(leg)) continue
      const sameDirection = !rowDirection || directionFromText(leg) === rowDirection
      const legThreshold = thresholdFromText(leg)
      const sameThreshold = !Number.isFinite(rowThreshold) || !Number.isFinite(legThreshold) || rowThreshold === legThreshold
      if (!sameDirection || !sameThreshold) continue
      const player = inferPlayerFromLeg(leg)
      if (player) return { ...row, Player: player }
    }
  }
  return row
}

function standaloneEntryFor(record = {}, fields = {}, resultFields = {}) {
  if (isParlayRow(fields)) return null
  const result = text(resultFields.Result || resultFields.Outcome)
  if (!result) return null
  const pick = firstValue(fields, ['Pick', 'Selection', 'Play', 'Prop'])
  if (!pick) return null
  return {
    pick,
    result,
    ...
    sheetName: recordSheetName(record),
    sheetRowNumber: recordSheetRowNumber(record),
    sourceName: firstValue(resultFields, ['Settlement Source']) || 'Same-date standalone result',
    sourceUrl: collectResultSourceUrls(resultFields)[0] || collectResultSourceUrls(fields)[0] || '',
    notes: firstValue(resultFields, ['Settlement Notes']) || `Reused same-date standalone verified result: ${pick}=${result}.`
  }
}
...
async function discoverSourcesFromSheetLegRows(row = {}, allRows = [], date = '', options = {}) {
  ...
  for (const leg of legs) {
    const match = allRows.find(candidateRecord => {
      ...
      return candidatePick && normalizedLegText(candidatePick) !== normalizedLegText(fullParlayPick) && sameLegText(candidatePick, leg)
    })
    if (!match) continue
...
export function calculateSettlementFields(row = {}, now = new Date(), options = {}) {
  ...
  if (profitPendingMissingOdds) {
    ...
    auditFields['Settlement Notes'] = 'Result verified, profit pending because odds are missing or invalid.'
  }
...
async function settleTable(key, config, allRows, date, now, options = {}) {
  ...
  const standaloneLegResults = options.standaloneLegResults || []
  ...
  for (const record of records) {
    ...
    if (!rawResult || /^pending$/i.test(rawResult)) {
      ...
      const settlementFields = enrichPlayerFromSameDateParlays(fields, allRows, date)
      const sourceUrls = collectResultSourceUrls(settlementFields)
      if (!canAttemptSourceSettlement(settlementFields, settleAll)) {
        skipped.push(skippedRecord(record, fields, 'Pending or blank Result and no result source URL', options))
        continue
      }

      let discovery = await discoverTrustedSourcesForPick(settlementFields, options)
      if (!discovery.urls?.length && isParlayRow(settlementFields)) {
        discovery = await discoverSourcesFromSheetLegRows(settlementFields, allRows, date, options) || discovery
      }
      ...
      const routed = await routeSettlementSources(settlementFields, {
        ...options,
        urls: discoveredUrls,
      ...
      const calculated = calculateSettlementFields(settlementFields, now, {
      ...
      const matchedItem = { record, fields: calculated.fields, reason: calculated.reason, discovery, verification: routed }
      matched.push(matchedItem)
      const standaloneEntry = standaloneEntryFor(record, settlementFields, calculated.fields)
      if (standaloneEntry) standaloneLegResults.push(standaloneEntry)
      continue
    }
    ...
    const matchedItem = { record, fields: calculated.fields, reason: calculated.reason }
    matched.push(matchedItem)
    const standaloneEntry = standaloneEntryFor(record, fields, calculated.fields)
    if (standaloneEntry) standaloneLegResults.push(standaloneEntry)
  }
