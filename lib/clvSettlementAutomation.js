...
function skippedRecord(record, fields, reason) {
  return {
    id: record.id,
  ...
  for (const record of records) {
    ...
    if (!rawResult || /^pending$/i.test(rawResult)) {
      if (shouldSkipNonOfficialRow(fields)) {
        skipped.push(skippedRecord(record, fields, 'Skipped non-official watchlist/pass row.'))
        continue
      }

      const sourceUrls = collectResultSourceUrls(fields)
      if (!canAttemptSourceSettlement(fields, settleAll)) {
        skipped.push(skippedRecord(record, fields, 'Pending or blank Result and no result source URL'))
        continue
      }
    ...
    const calculated = calculateSettlementFields(fields, now)
    if (calculated.skipped) {
      skipped.push(skippedRecord(record, fields, calculated.reason))
      continue
    }
    matched.push({ record, fields: calculated.fields, reason: calculated.reason })
  }
