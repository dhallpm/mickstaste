...
function clvField(value) {
  if (!Number.isFinite(value)) return { '%CLV': '' }
  return { '%CLV': Number(value.toFixed(2)) }
}
...
export function calculateClvFields(row = {}) {
  ...
  const postedNumber = parseNumber(firstValue(row, ['Closing Number', 'Best Number', 'Posted Number', 'Line', 'Number', 'Suggested Line']))
  ...
  let closingOdds = parseAmericanOdds(firstValue(row, ['Closing Odds', 'Closing Price']))

  if (moneyline && !Number.isFinite(closingOdds) && Number.isFinite(verifiedClosingNumber) && Math.abs(verifiedClosingNumber) >= 100) {
    closingOdds = verifiedClosingNumber
  }
  ...
  if (moneyline) {
    if (Number.isFinite(postedOdds) && Number.isFinite(closingOdds)) {
      ...
      if (Number.isFinite(postedDecimal) && Number.isFinite(closingDecimal) && closingDecimal > 0) {
        ...
        return { fields: clvField(percent), reason: 'moneyline odds' }
      }
    }
    return { fields: {}, reason: 'moneyline needs Closing Odds' }
  }
  ...
  if (Number.isFinite(verifiedClosingNumber) && Number.isFinite(postedNumber)) {
    ...
    let value
    if (direction === 'over') value = verifiedClosingNumber - postedNumber
    else if (direction === 'under') value = postedNumber - verifiedClosingNumber
    else if (postedNumber < 0 || verifiedClosingNumber < 0) value = Math.abs(verifiedClosingNumber) - Math.abs(postedNumber)
    else if (postedNumber > 0 || verifiedClosingNumber > 0) value = postedNumber - verifiedClosingNumber
    else value = verifiedClosingNumber - postedNumber

    return { fields: clvField(value), reason: 'line' }
  }
  ...
  if (Number.isFinite(postedOdds) && Number.isFinite(closingOdds)) {
    ...
    if (Number.isFinite(postedDecimal) && Number.isFinite(closingDecimal) && closingDecimal > 0) {
      const percent = (postedDecimal / closingDecimal - 1) * 100
      return { fields: clvField(percent), reason: 'odds' }
    }
  }
...
export async function recalculateClv(options = {}) {
  ...
  for (const record of records) {
    ...
    const hasClosing = text(firstValue(fields, ['Verified Closing Number'])) || text(firstValue(fields, ['Closing Odds', 'Closing Price']))
    if (!hasClosing) {
      skipped.push({ id: record.id, reason: 'No Verified Closing Number or Closing Odds' })
      continue
    }
...
export async function settleResults(options = {}) {
  ...
  for (const [key, config] of Object.entries(TABLE_CONFIGS)) {
    try {
      const result = await settleTable(key, config, date, now, options)
