...
function setupInstructions(missingTables = [], missingFields = [], missingViews = []) {
  const lines = [
...
function nextStepsFor({ setupComplete, schemaReadAvailable, errors, missingTables, missingFields, missingViews }) {
  if (setupComplete) {
    return [
  ...
  const steps = [
    'Keep Google Sheets fallback active until setupComplete is true.',
    'Re-run /api/setup-airtable-base after correcting Airtable token scopes or manual schema gaps.'
  ]

  if (!schemaReadAvailable) {
...
export async function setupAirtableBase(options = {}) {
  ...
  report.instructions = report.setupComplete
    ...
    : setupInstructions(report.missingTables, report.missingFields, report.missingViews)
