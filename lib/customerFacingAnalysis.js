...
function cleanCustomerText(text = '') {
  ...
  if (!raw || containsInternalAnalysisText(raw)) return ''
  return raw
}
...
export function buildPublicPreview(row = {}) {
  ...
  const injury = cleanCustomerText(value(row, ['Injury Notes', 'injuryNotes']))
  ...
  return paragraphize([
    `Opening preview: ${pick} is on the public card for ${game} because the ${betType} angle has a clear betting question: can this side of the market still hold up at the posted number once matchup, price, and availability are checked together? ${writeup ? sentence(writeup) : ''}`,
    `Matchup preview: ${marketLanguage(row)} ${marketNotes ? `The current notes point to this specific angle: ${sentence(marketNotes)}` : 'The public view keeps the handicap focused on the matchup shape without exposing the full VIP breakdown.'}`,
    `Market preview: ${line ? `The playable range is tied to ${line}.` : 'The number still has to be confirmed against the live board before it should be treated as a playable edge.'} ${injury ? `Availability note: ${sentence(injury)}` : 'Late availability news can still change the value, so the final card should be checked close to release.'} ${review ? 'The public card should wait for a final board check before release.' : 'VIP analysis handles the deeper market, risk, and verdict details.'}`
  ])
}

export function sanitizePublicWriteup(row = {}) {
  const current = value(row, ['Writeup', 'writeup', 'Card Description', 'description'])
  return hasStrongPublicPreview(current) ? current : buildPublicPreview(row)
}
...
export function sanitizeCustomerFacingAnalysis(row = {}) {
  ...
  return hasStrongVipAnalysis(current) ? current : buildCustomerFacingAnalysis(row)
}
