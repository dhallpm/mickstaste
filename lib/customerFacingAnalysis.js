...
export function buildCustomerFacingAnalysis(row = {}) {
  ...
  const paragraphs = [
    ...
    noBetCutoff
      ? `Risk note: the best number is ${bestNumber || 'the listed line'}, with a no-bet cutoff of ${noBetCutoff}; beyond that point, the edge is no longer strong enough for the card.`
      : bestNumber
      ? `Risk note: ${bestNumber} is the preferred number, and meaningful movement away from that price should reduce the edge.`
      : 'Risk note: the play depends on the listed market staying close to the current edge, so aggressive line movement should be treated cautiously.',
