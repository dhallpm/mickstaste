...
function isParlayPick(row = {}) {
  ...
  return /\bparlay\b|\blegs?\b/.test(value) || /\|/.test(firstValue(row, ['Pick', 'Selection', 'Play']))
}

function inferLeagueForPick(row = {}, fallback = '') {
  const explicit = firstValue(row, ['League', 'Sport'])
  if (leagueFamily(explicit)) return explicit
  const value = [
    firstValue(row, ['Pick', 'Selection', 'Play']),
    firstValue(row, ['Game', 'Matchup', 'Event']),
    fallback
...
async function discoverMlbOfficial({ game, date, player }, options = {}) {
  ...
  const gameMatch = games.find(candidate => mlbGameMatches(candidate, teamCandidates))
  if (!gameMatch) return []
...
async function discoverEspn({ family, game, date, player }, options = {}) {
  ...
  const rowTeams = inferTeamsFromGame(game)
  const teamCandidates = rowTeams.length ? rowTeams : inferTeamCandidates({ Game: game, Pick: game })
  if (!path || !teamCandidates.length) return []
...
export async function discoverPlayerStatSources({ league, game, player, date, betType, prop }, options = {}) {
  ...
  if (!family || !dateKey(date) || (!teamCandidates.length && !(family === 'nhl' && resolvedPlayer))) return []
...
export async function discoverTrustedSourcesForPick(row = {}, options = {}) {
  ...
  if (isParlayPick(row)) {
    ...
    for (const leg of splitParlayLegs(row)) {
      const legRow = {
        ...
        Market: '',
        Category: '',
        League: inferLeagueForPick({ ...row, Pick: leg, Game: leg }, league),
  ...
  const sources = isPlayerStatPick(row)
    ...
    : await discoverGameBoxScoreSources({ league: resolvedLeague, game: resolvedGame, date }, options)
