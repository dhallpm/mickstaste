...
function sameLegText(left = '', right = '') {
  const a = normalizeLeg(left)
  const b = normalizeLeg(right)
...
function playerName(row = {}) {
  ...
  return text(pick)
    .replace(/\s*(?:-|\u2013)\s*/g, ' ')
    .replace(/\b(over|under|anytime goal|strikeouts?|hrr|hits?\s*\+\s*runs?\s*\+\s*rbi|shots?\s+on\s+goal|sog).*$/i, '')
    .trim()
}
...
function classifyMarket(row = {}) {
  ...
  if (/\bdraw no bet\b|\bdnb\b|\bno draw\b|\btie no bet\b/.test(value)) return 'drawNoBet'
  if (/\bdouble chance\b/.test(value)) return 'doubleChance'
  ...
  if (/\bshots?\s+on\s+goal\b|\bsog\b/.test(value)) return 'shotsOnGoal'
...
function parsePlayerStats(content = '', row = {}) {
  ...
  const shotsOnGoal = window.match(/\b(?:shots?\s+on\s+goal|sog|shots?)\s*[:=-]?\s*(\d{1,2})\b/i) ||
    window.match(/\b(\d{1,2})\s*(?:shots?\s+on\s+goal|sog|shots?)\b/i)
  if (shotsOnGoal) stats.shotsOnGoal = Number(shotsOnGoal[1])
...
function resultFromScore(row = {}, finalScore = {}) {
  ...
  if (market === 'doubleChance') {
    const pick = normalize(firstValue(row, ['Pick', 'Selection', 'Play', 'Team', 'Side']))
    const drawProtected = /\bdraw\b|\btie\b|\bx\b/.test(pick)
    if (teamScore === opponentScore && drawProtected) return 'Win'
    if (teamScore === opponentScore) return 'Push'
    return teamScore > opponentScore ? 'Win' : 'Loss'
  }
...
function resultFromPlayerStats(row = {}, playerStats = {}) {
  ...
  if (market === 'shotsOnGoal') return compareOverUnder(Number(stats.shotsOnGoal), threshold(row), direction(row))
  return ''
}
...
function findStandaloneLegResult(leg = '', options = {}) {
  const entries = options.standaloneLegResults || []
  return entries.find(entry => sameLegText(entry.pick, leg))
}

function verifyStandaloneConflict(leg = '', standalone = {}, providerResults = [], options = {}) {
  const directLeg = providerResults
    .flatMap(providerResult => (providerResult.legResults || []).map(legResult => ({ ...legResult, providerResult })))
    .find(legResult => sameLegText(legResult.leg, leg) && legResult.result)
  if (directLeg && directLeg.result !== standalone.result) {
    return {
      leg,
      result: '',
      status: 'needs_review',
      sourceName: directLeg.providerResult.sourceName,
      sourceUrl: directLeg.providerResult.sourceUrl,
      notes: `Standalone result conflict for ${leg}: same-date row ${standalone.pick}=${standalone.result}, source route=${directLeg.result}.`
    }
  }

  if (!standalone.fields) return null
  const standalonePick = firstValue(standalone.fields, ['Pick', 'Selection', 'Play', 'Prop']) || leg
  const row = {
    ...standalone.fields,
    Pick: standalonePick,
    Selection: standalonePick,
    Play: standalonePick
  }
  const verified = providerResults
    .map(providerResult => ({
      providerResult,
      result: verifyRowAgainstProvider(row, providerResult, options)
    }))
  ...
  if (!verified || verified.result.result === standalone.result) return null
  return {
    leg,
    result: '',
    status: 'needs_review',
    ...
    notes: `Standalone result conflict for ${leg}: same-date row ${standalone.pick}=${standalone.result}, source route=${verified.result.result}.`
  }
}
...
function verifyRowAgainstProvider(row = {}, providerResult = {}, options = {}) {
  ...
  let result = ''
  if (['strikeouts', 'hrr', 'anytimeGoal', 'shotsOnGoal'].includes(market)) {
...
function aggregateParlayResult(row = {}, providerResults = [], options = {}) {
  const legResults = splitParlayLegs(row).map(leg => {
    const standalone = findStandaloneLegResult(leg, options)
    if (standalone) {
      const conflict = verifyStandaloneConflict(leg, standalone, providerResults, options)
      if (conflict) return conflict
      return {
        leg,
        result: standalone.result,
        status: 'verified',
        sourceName: standalone.sourceName || 'Same-date standalone result',
        sourceUrl: standalone.sourceUrl || '',
        notes: `Reused same-date standalone result: ${standalone.pick}=${standalone.result}.`
      }
    }
  ...
  if (legResults.some(leg => leg.status !== 'verified' || !leg.result)) {
    const reviewNotes = legResults
      .filter(leg => leg.status !== 'verified' || !leg.result)
      .map(leg => leg.notes || `${leg.leg} could not be verified`)
      .join('; ')
    return {
      status: 'needs_review',
      result: '',
      ...
      playerStats: {},
      legResults,
      notes: reviewNotes || 'One or more parlay legs could not be verified.'
    }
  }
  if (legResults.some(leg => leg.result === 'Push' || leg.result === 'Void')) {
