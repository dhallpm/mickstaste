...
export function archiveFields(row = {}) {
  ...
  // Do not write unit strings into the legacy Profit/Loss field because some Airtable bases
  // have that field configured as Currency. Website/API reads Profit/Loss Units first.
...
export async function archiveClosedBets(options = {}) {
  ...
  for (const config of ACTIVE_AIRTABLE_TABLE_CONFIG) {
    ...
    for (const record of records) {
      ...
      archived.push({
        ...
        pick: row.Pick || row.Selection || row.Play || row['Card Title'] || row.Player || row.Game || '',
  ...
  await logSyncAction('Archive closed bets', {
    ...
    message: options.dryRun ? 'Dry run only' : 'Finalized picks archived with Closed status and unit P/L'
  })
