...
function isVipMasterPick(row = {}) {
  if (isPropsTable(row) || isLottoTable(row) || isLongshotsTable(row)) return false
  const value = text(row, ['Access', 'access', 'Tier', 'Access Tier', 'Category', 'category', 'targetTable', 'vip'])
  return has(/\b(vip|premium|vip vault|true|yes)\b/i, value)
}
...
export function routePickCategory(row = {}) {
  ...
  if (isVipMasterPick(row)) {
    return { activeTable: 'Master Picks', archiveTable: 'Results Archive', websiteSection: 'vip' }
  }
