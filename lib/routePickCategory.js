...
function categoryText(row = {}) {
  return normalizePickCategory(text(row, ['__table', 'Parlay Type', 'Category', 'Type', 'Market', 'Bet Type', 'Pick', 'Description']))
}
...
export function isActiveVisible(row = {}, now = new Date()) {
  ...
  const lottoStatusHasGrade = isGradeOnly(statusOnly) && routePickCategory(row).websiteSection === 'lotto'
  const statusOk = !status || has(/\b(active|posted|released|open|pregame|pending)\b/i, status) || lottoStatusHasGrade
