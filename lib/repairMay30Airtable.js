...
function rowKey(row = {}) {
  return text(row['Record Key']).toLowerCase()
}
...
export function buildMay30RepairPlan(masterRows = [], archiveRows = []) {
  ...
  for (const rows of byKey.values()) {
    ...
    duplicateDeletes.push(...rows.filter(row => row.id !== survivor.id))
...
export async function repairMay30Airtable(options = {}) {
  ...
  if (!options.dryRun) {
    ...
    for (const row of plan.duplicateDeletes) {
      await deleteAirtableRecord(master.tableName, row.id)
    }
  }
  ...
  return {
    ...
    deletedDuplicates: plan.duplicateDeletes.map(row => ({ id: row.id, pick: row.Pick, recordKey: row['Record Key'] })),
