...
const LOCATOR_CONFIRMED_DUPLICATES = [
  { id: 'rec7g1bFsiNlSceH7', Pick: 'Braves ML', 'Record Key': '2026-05-30|mlb|braves @ reds|braves ml|moneyline|vip|-150' },
  { id: 'recVIBazCqyLcaIbc', Pick: 'Spurs +3.5', 'Record Key': '2026-05-30|nba|spurs @ thunder|spurs +3.5|spread|free|-110' },
  { id: 'recQ3pCQPtmd8qA8b', Pick: 'Yankees ML', 'Record Key': '2026-05-30|mlb|yankees @ athletics|yankees ml|moneyline|vip|-170' },
  { id: 'recSBKBuyrIMQk8Co', Pick: 'Yankees Team Total Over 5.0', 'Record Key': '2026-05-30|mlb|yankees @ athletics|yankees team total over 5.0|team total|free|-110' }
]
...
export function buildMay30RepairPlan(masterRows = [], archiveRows = []) {
  ...
  const plannedDeleteIds = new Set(duplicateDeletes.map(recordId))
  for (const row of LOCATOR_CONFIRMED_DUPLICATES) {
    if (!plannedDeleteIds.has(recordId(row))) duplicateDeletes.push(row)
  }
