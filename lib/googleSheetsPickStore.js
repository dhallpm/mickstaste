...
function excludedResultState(value = '') {
  return /^(pending|watchlist|pass)$/i.test(cleanText(value))
}

function closedSettlementState(value = '') {
  return /^(closed|settled|graded|complete|completed|final)$/i.test(cleanText(value))
}
...
export function inferGoogleSheetsSettlementResult(row = {}) {
  ...
  return rowValues(fields, ['Settlement Notes', 'Short Take'])
...
export function isGoogleSheetsSettledPickRow(row = {}) {
  const fields = row.fields || row
  const primaryResult = firstRowValue(fields, ['Result', 'Outcome', 'Final Result', 'Pick Result', 'Graded Result'])
  const primaryStatus = firstRowValue(fields, ['Status', 'Display Status', 'Pick Status'])
  if (excludedResultState(primaryResult) || excludedResultState(primaryStatus)) return false

  const profitLoss = firstRowValue(fields, ['Profit/Loss', 'P/L', 'PL', 'Profit Loss', 'Profit / Loss', 'Profit-Loss', 'Profit/Loss Units'])
  const hasClosedStatus = rowValues(fields, ['Status', 'Display Status', 'Pick Status', 'Settlement Status'])
    .some(closedSettlementState)

  return Boolean(inferGoogleSheetsSettlementResult(fields) || cleanText(profitLoss) || hasClosedStatus)
}
