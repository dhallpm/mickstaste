...
export function isGoogleSheetsPublicResultRow(row = {}) {
  const fields = row.fields || row
  const result = rowValues(fields, ['Result', 'Outcome'])
    .map(finalResultValue)
    .find(Boolean)
  const profitLoss = firstRowValue(fields, ['Profit/Loss', 'P/L', 'PL', 'Profit Loss', 'Profit / Loss', 'Profit-Loss', 'Profit/Loss Units'])
  const settledAt = firstRowValue(fields, ['Settled At'])
  const settlementStatus = firstRowValue(fields, ['Settlement Status'])

  return Boolean(
    result &&
    cleanText(profitLoss) &&
    cleanText(settledAt) &&
    /^settled$/i.test(cleanText(settlementStatus))
  )
}
...
export async function listResultsGoogleSheetsPicksWithWarnings(options = {}) {
  const sheets = options.sheets || await sheetsClient()
  const rows = []
  const warnings = []
  const loadedTabs = []
...
export async function listSettledGoogleSheetsPicksWithWarnings(options = {}) {
  const result = await listResultsGoogleSheetsPicksWithWarnings(options)
  const rows = result.rows.filter(isGoogleSheetsPublicResultRow)
  return {
    ...result,
    rows,
    scannedRowCounts: Object.fromEntries((result.loadedTabs || []).map(tab => [
      tab,
      result.rows.filter(row => row.__table === tab).length
    ])),
    resultRowCounts: Object.fromEntries((result.loadedTabs || []).map(tab => [
      tab,
...
export default {
  ...
  listResultsGoogleSheetsPicksWithWarnings,
