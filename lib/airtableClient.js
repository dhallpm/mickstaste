...
export const AIRTABLE_TABLES = {
  ...
  lottoProps: process.env.AIRTABLE_LOTTO_PROPS_TABLE_ID || process.env.AIRTABLE_LOTTO_PROPS_TABLE || '',
...
export const AIRTABLE_TABLE_RESOLVERS = {
  ...
  lottoParlays: {
    ...
    aliases: ['Lotto Parlay', 'Lottery Parlay', 'Lottery Parlays', 'Lotto Props', 'Parlays', 'Safe Lotto Parlays'],
    required: false
  },
  lottoProps: {
    key: 'lottoProps',
    preferredIdEnvVar: 'AIRTABLE_LOTTO_PROPS_TABLE_ID',
    preferredEnvVar: 'AIRTABLE_LOTTO_PROPS_TABLE',
    defaultName: 'Lotto Props',
    aliases: ['Lotto Prop Parlays'],
    required: false
  },
  longshots: {
    key: 'longshots',
...
export const ACTIVE_AIRTABLE_TABLES = [
  ...
  ...(AIRTABLE_TABLES.lottoProps ? [AIRTABLE_TABLES.lottoProps] : []),
...
export const ACTIVE_AIRTABLE_TABLE_CONFIG = [
  ...
  ...(AIRTABLE_TABLES.lottoProps ? [AIRTABLE_TABLE_RESOLVERS.lottoProps] : []),
...
function airtableBasePrefix() {
  const baseId = defaultAirtableBaseId()
  return baseId ? `${baseId.slice(0, 6)}...` : ''
}

function verifiedAirtableBaseId() {
  return String(process.env.AIRTABLE_VERIFIED_BASE_ID || KNOWN_AIRTABLE_BASE_ID).trim()
}

export function resolveAirtableBaseIds() {
  return Array.from(new Set([verifiedAirtableBaseId()]
    .map(baseId => String(baseId || '').trim())
    .filter(Boolean)))
}

function defaultAirtableBaseId() {
  return resolveAirtableBaseIds()[0] || requiredEnv('AIRTABLE_BASE_ID')
}
...
function tableEnvVar(tableName) {
  const configured = {
    ...
    ...(AIRTABLE_TABLES.lottoProps ? { [AIRTABLE_TABLES.lottoProps]: isAirtableTableId(AIRTABLE_TABLES.lottoProps) ? 'AIRTABLE_LOTTO_PROPS_TABLE_ID' : 'AIRTABLE_LOTTO_PROPS_TABLE' } : {}),
...
function airtableUrl(tableName, params = {}, baseId = defaultAirtableBaseId()) {
...
export async function listAirtableRecords(tableName, options = {}) {
  ...
  do {
    ...
    records.push(...(payload.records || []).filter(record => !isBlankAirtableRecord(record)))
...
function hasMeaningfulAirtableValue(value) {
  if (value === undefined || value === null || value === false) return false
  if (Array.isArray(value)) return value.some(hasMeaningfulAirtableValue)
  if (typeof value === 'object') return Object.values(value).some(hasMeaningfulAirtableValue)
  return String(value).trim() !== ''
}

export function isBlankAirtableRecord(record = {}) {
  return !Object.values(record.fields || {}).some(hasMeaningfulAirtableValue)
}
...
export async function listAirtableRecordsFromResolvedTable(config, options = {}) {
  ...
  const configuredBaseId = String(process.env.AIRTABLE_BASE_ID || '').trim()
  const verifiedBaseId = verifiedAirtableBaseId()

  for (const baseId of baseIds) {
    for (const tableName of candidates) {
      try {
        const records = await listAirtableRecords(tableName, { ...options, baseId })
        if (configuredBaseId && baseId === verifiedBaseId && baseId !== configuredBaseId) {
          warnings.push('AIRTABLE_BASE_ID differs from the verified Airtable base. Used the verified base first; correct AIRTABLE_BASE_ID in the Vercel production environment.')
        }
...
const AIRTABLE_FRAGILE_OPTIONAL_FIELDS = new Set([
  'Confidence',
  'EV Edge',
  'CLV',
  'No Bet Cutoff',
...
export function cleanAirtableFields(fields = {}) {
  ...
  for (const fieldName of AIRTABLE_FRAGILE_OPTIONAL_FIELDS) {
    delete cleaned[fieldName]
  }
  if (cleaned.Odds === '' || cleaned.Odds === null || cleaned.Odds === undefined) {
    delete cleaned.Odds
  }
...
function extractRejectedAirtableField(error) {
  if (error?.statusCode !== 422 && error?.airtableStatus !== 422) return ''
  const message = String(error?.message || '')
  return message.match(/field\s+name:\s*"([^"]+)"/i)?.[1] ||
    message.match(/field\s+"([^"]+)"/i)?.[1] ||
    message.match(/field\s+([A-Za-z0-9 /#_-]+)/i)?.[1]?.trim() ||
    ''
}

function removeFieldFromPayloadRecords(records, fieldName) {
  let removed = false
  ...
  return { records: nextRecords, removed }
}

async function airtableFetchWithFieldRetry(tableName, request, options = {}) {
  let body = request.body
  const removedFields = new Set()

  for (;;) {
    try {
      const payload = await airtableFetch(tableName, { ...request, body })
      for (const fieldName of removedFields) {
        options.warnings?.push(`Removed Airtable-incompatible field ${fieldName} and retried successfully.`)
      }
      return payload
    } catch (error) {
      const fieldName = extractRejectedAirtableField(error)
      if (!fieldName || removedFields.has(fieldName) || removedFields.size >= 25 || !Array.isArray(body?.records)) {
        throw error
      }

      const next = removeFieldFromPayloadRecords(body.records, fieldName)
...
export async function createAirtableRecords(tableName, fieldsList, options = {}) {
  ...
  for (let i = 0; i < fieldsList.length; i += 10) {
    const payload = await airtableFetchWithFieldRetry(tableName, {
      baseId: options.baseId,
      method: 'POST',
      body: {
