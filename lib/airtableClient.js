...
export async function listTodayAirtablePicks(date = new Date()) {
  ...
  return records.filter(row => {
    const rowDate = row.Date || row.date || row['Game Date'] || row.Timestamp
    const parsed = rowDate ? new Date(rowDate) : null
    return parsed && !Number.isNaN(parsed.getTime()) && parsed.toISOString().slice(0, 10) === isoDate
  })
}
