...
export const AIRTABLE_TABLES = {
  masterPicks: process.env.AIRTABLE_MASTER_PICKS_TABLE_ID || process.env.AIRTABLE_MASTER_PICKS_TABLE || 'Master Picks',
...
export function resolveAirtableTableName({ preferredIdEnvVar, preferredEnvVar, defaultName, aliases = [] }) {
  ...
  return Array.from(new Set([configuredId, configuredName, defaultName, ...aliases]
...
export function airtableMissingTableDetails(tableName) {
  ...
  return {
    requestedTable: tableRefForMessage(tableName),
...
export async function listAirtableRecordsFromResolvedTable(config, options = {}) {
  ...
  for (const tableName of candidates) {
    try {
      ...
      warnings.push(`Airtable table ${tableRefForMessage(tableName)} not found; tried next alias.`)
    }
  }
