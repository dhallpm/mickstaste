...
const TABLE_ALIAS_GROUPS = {
  ...
  lotto: ['Lotto Parlays', 'Lotto Parlay', 'Lottery Parlay', 'Lottery Parlays', 'Lotto Props', 'Parlays', 'Safe Lotto Parlays'],
...
async function testRecordsApiTable(baseId, tableName) {
...
async function diagnoseAliasGroup(aliasesOrConfig) {
  ...
  for (const baseId of baseIds) {
    for (const alias of aliases) {
      const result = await testRecordsApiTable(baseId, alias)
      ...
      if (result.ok) {
        return {
          ...
          baseIdPrefix: `${baseId.slice(0, 6)}...`,
          usedConfiguredBase: baseId === process.env.AIRTABLE_BASE_ID,
          recordCountSampled: result.recordCountSampled,
          attempted: attempts.map(item => `${item.baseIdPrefix}/${item.tableName}`),
          attempts
        }
      }
    }
