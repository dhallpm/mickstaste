...
async function diagnoseAliasGroup(aliases) {
  const attempts = []
...
export async function runAirtableDiagnostics() {
  ...
  const tables = {}
  for (const [groupName, aliases] of Object.entries(TABLE_ALIAS_GROUPS)) {
    tables[groupName] = await diagnoseAliasGroup(aliases)
  }
